...
async function operateSaveDialog(action, destination = '', reviewed = false, expectNoFile = false, recovery = false) {
  const args = ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', resolve(root, 'tests/native/native-save-dialog.ps1'),
...
try {
  ...
  const { qualifyReviewedExport } = await import(pathToFileURL(resolve(root, 'tests/native/native-reviewed-export.mjs')).href);
  await qualifyReviewedExport({ page, data, output, operateSaveDialog, createWritingProject, checks });
  const { qualifyReviewedEvidence } = await import(pathToFileURL(resolve(root, 'tests/native/native-reviewed-evidence.mjs')).href);
  await qualifyReviewedEvidence({ page, data, output, createWritingProject, checks });
  const { qualifyReviewedSummaries } = await import(pathToFileURL(resolve(root, 'tests/native/native-reviewed-summaries.mjs')).href);
  await qualifyReviewedSummaries({ page, data, output, createWritingProject, checks });
  const { qualifyStructuredSuggestions } = await import(pathToFileURL(resolve(root, 'tests/native/native-structured-suggestions.mjs')).href);
  await qualifyStructuredSuggestions({ page, data, output, createWritingProject, checks });
  const { runPromiseHistoryFlow } = await import(pathToFileURL(resolve(root, 'tests/native/native-promise-history.mjs')).href);
  await runPromiseHistoryFlow({ page, data, output, createWritingProject, checks });
  const { runKnowledgeFlow } = await import(pathToFileURL(resolve(root, 'tests/native/native-knowledge.mjs')).href);
  await runKnowledgeFlow({ page, data, output, createWritingProject, checks });
  const { qualifyContextLookup } = await import(pathToFileURL(resolve(root, 'tests/native/native-context-lookup.mjs')).href);
  await qualifyContextLookup({ page, output, testRoot: data, createWritingProject, checks });
  const { qualifyRecoveryCopy } = await import(pathToFileURL(resolve(root, 'tests/native/native-recovery-copy.mjs')).href);
