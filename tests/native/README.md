...
- reviewed continuation preparation over real IPC: exact earlier reviewed chapters, a separate working target, future/private exclusion, frozen packet inspection, stale-basis refusal, and policy revocation; and
...
- strict full-flow clipboard copy/paste coverage, which remains part of the current 34-check smoke and is not omitted from CI.

The source-level checks cover the same restricted document and identity contract with shared golden fixtures. Current schema 16 retains the exact reviewed-context manifests and immutable reader-position pins introduced for author-only review, import history, provider results, writing-brief drafts, persistent discussion source choices, guidance, durable discussion retry, proposals, and export records, and adds the C4-A chapter-memory job/result/view records. The reviewed-context check prepares a packet without a model call; C4-A generated views are inspection-only and are not supplied to later model packets. Native Windows UI Automation exposes the manuscript textbox, feedback labels, discussion controls, guidance form/actions, context inspector, and proposal cards. Unicode input is internal robustness evidence for English authoring, not a separate language feature. Latest results and exact executable evidence are maintained in [implementation status](../../docs/IMPLEMENTATION_STATUS.md); the chronology below is historical.

The tracked native harness contains 34 checks, including three C4-A checks. [CI 34003001409](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/34003001409) passed all 34 at checkpoint `7a27135`, with no errors. The separate final local diagnostic passed 33/34 at `2026-09-06T01:01:50.070Z`, omitting only the documented local OS clipboard case. Exact executable and separate live-memory evidence are recorded in implementation status. Installed-package and narrative-quality gates remain separate.
