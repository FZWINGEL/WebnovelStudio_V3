...
**Status:** confirmed product direction; native Library/Workspace persistence is implemented at the current W1–W3 boundary, while W0 and broader qualification remain open, 5 September 2026.
...
The default native UI opens a persistent Library/Workspace with blank projects and optional chapter, character, world, theme, hook, scene, and note documents. It wires autosave through Rust-owned SQLite, flush-before-switch, project/document rename, duplicate, archive/unarchive, and native folder, backup, recovery, and draft-TXT dialogs. W1 structural scope validation and shared JS/Rust fixtures are implemented; W2 session/core receipts and reconciliation are implemented; W3 schema-3 migration protects upgrades with a durable pre-upgrade backup, recovers schema-1/2 databases, and carries exact-head caret/last-document state, metadata compare-and-swap, and `context_source_epoch`. Copied historical snapshots cannot authorize new project operations.

The Story Context core currently has C0's pure contracts and 16 adversarial tests plus C1's working-basis Rust snapshot/retrieval actor. C1 retains original source, performs exact literal/lexical scans and source-only alias matches, returns Unicode UTF-16 spans, tracks revocation and source staleness conservatively, retries snapshots across restart, and rebuilds a disposable per-document index with yielding. Aliases remain private to AuthorRoom until safe grants; reviewed/history/character policies are unavailable until authority work, and AuthorRoom Revise/Continue is blocked. C2's pure packet compiler is in progress; no context UI or provider integration is claimed.
