...
The Story Context Engine retains original evidence, freezes each request's permitted sources, and records the exact input sent to the assistant. You can also review and retain an exact chapter version without running a model. The core can freeze an exact reviewed basis consisting of earlier reviewed chapters plus the current working target over IPC; its continuation UI, live dispatch, and ready export remain open. Generated memory, accepted story facts, reviewed generation, broader providers, and full release qualification remain open. See [implementation status](docs/IMPLEMENTATION_STATUS.md) for executed checks and [Codex qualification](docs/CODEX_QUALIFICATION.md) for the precise live-provider boundary.
