...
**In development.** The persistent workspace runs locally, and discussion currently uses a clearly labelled deterministic test model. The selected-passage proposal/review/Apply slice now also supports an optional author-approved writing brief in local development. The latest full wrapper checkpoint passed rustfmt, workspace Clippy with `-D warnings`, **296 active Rust tests** (285 core and 11 desktop; one ignored subprocess target), **185 frontend tests**, TypeScript/Vite, and the embedded Tauri build. The latest native diagnostic passed 27 of 28 checks on WebView2 `152.0.4191.62` at `2026-09-05T21:52:10.597Z`; only the known local clipboard check is omitted. The current tracked harness contains 28 checks, and this diagnostic covered model choice, favorites, trait persistence across reload, and blocked Codex dispatch without substitution. The build is 638.70 KB CSS/JS output with the existing Vite size warning. Broader installed-release qualification and production live AI remain unfinished. The separate native Codex discovery and synthetic experiments are recorded in [Codex qualification](docs/CODEX_QUALIFICATION.md). See [implementation status](docs/IMPLEMENTATION_STATUS.md) for exact test evidence and remaining work.

The model preference implementation is locally covered by seven Rust core tests, five selector/Settings tests, two FeedbackPanel binding tests, and one desktop model-binding test. Only the local Mock model is ready; unavailable Codex choices are retained as preferences but block generation, with no CLI probes, credential reads, or new live requests. The strict native harness verified model choice, favorites, High/Fast trait persistence across reload, exact Rust IPC state, and no silent Codex substitution.
...
- Persistent document discussion, selected-passage feedback, unsent drafts, Stop, and explicit retry using the local test model. Queued Stop seals immediately; running Stop records cleanup intent and settles with retained partial output. Failed local response saves remain inspectable with a visible retry action, and no generation is replayed. Unchanged retries keep their original one-use guidance, and the retry choice survives restart.
...
- **This W5 checkpoint:** suggest edits for a selected chapter passage with deterministic review cards, editable previews, Apply, and Reject. The writing-brief flow passed the strict 26-check hosted native qualification.
...
- **Current W8 foundation slice:** bounded Windows child-process observation and pure Claude/Codex JSONL parser contracts are locally tested, but no live provider dispatch or model qualification is enabled.
- **Model selection foundation:** the Rust library stores model preferences in schema 2 while author projects remain schema 11. The header selector and Settings surface expose the local Mock and reference-only Codex catalog; only Mock can dispatch.

Guidance is an explicit author choice and never changes manuscript text or establishes canon. Requests retain the exact instruction versions and prior exchanges they used. Restricted chapter-passage edit requests use a reader frontier and exclude author-room private material, future material, current guidance, saved discussion sources, and recent chat. An optional brief is sent as exact `approvedWritingBrief` text only after explicit approval; its origin ID, private chat, pins, and guidance stay local, and editing the text or scope clears approval. Generated memory digests and bounded live-provider lookups remain planned parts of the [Story Context Engine](docs/V3_STORY_CONTEXT_SYSTEM.md). The V2 Codex adapter/model-picker remains an authorized reference; [Codex qualification](docs/CODEX_QUALIFICATION.md) records native discovery and bounded synthetic experiments, while production discussions remain mock-only and live-provider support stays unqualified.
