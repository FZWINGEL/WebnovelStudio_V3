...
The latest remote CI run [33971040177](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33971040177), for commit `b8d4cb8`, reached the UI but failed its native smoke timeout while waiting for a CSS-hidden diagnostic label. The wait/query logic is fixed locally and the remote rerun is pending. Its contracts on both OSes, Windows Clippy/tests, and native build passed; no rerun result is claimed here.
