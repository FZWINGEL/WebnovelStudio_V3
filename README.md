...
- **Passage-backed reviewed story details (schema-21 local slice):** while reviewing a chapter, select a passage, record what object or holder it describes, choose whether the detail is visible to the reader, and inspect the reviewed evidence later. The local source checks and final native development diagnostic pass 39/40, omitting only the known local OS clipboard case; hosted CI qualification remains pending. This remains reviewed evidence, not automatic canon or a complete state extraction. See the [reviewed story evidence contract](docs/ADR_0018_REVIEWED_STORY_EVIDENCE.md).
