...
The Story Context Engine retains original evidence, freezes each request's permitted sources, and records the exact input sent to the assistant. Generated digests, reviewed-story authority, richer retrieval, broader provider support, and full release qualification remain open. See [implementation status](docs/IMPLEMENTATION_STATUS.md) for executed checks and [Codex qualification](docs/CODEX_QUALIFICATION.md) for the precise live-provider boundary.
