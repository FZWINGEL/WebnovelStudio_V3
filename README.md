...
**In development.** The persistent workspace runs locally, and discussion currently uses a clearly labelled deterministic test model. The selected-passage proposal/review/Apply slice now also supports an optional author-approved writing brief in local development. The final embedded Tauri build passed 26 of 27 local native diagnostic checks on WebView2 `152.0.4191.62` at `2026-09-05T21:29:29.901Z`; only clipboard is omitted. The diagnostic includes a real SQLite fault, reload, local retry saving the response, and no extra discussion run, with the recovery notice and retry action visible. [CI run 33991463598](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33991463598) passed Windows/Ubuntu contracts and all 26 strict native checks, including writing briefs and clipboard, on WebView2 `151.0.4129.101`. Installed-release qualification and production live AI remain unfinished. The separate native Codex discovery and synthetic experiments are recorded in [Codex qualification](docs/CODEX_QUALIFICATION.md). See [implementation status](docs/IMPLEMENTATION_STATUS.md) for exact test evidence and remaining work.
