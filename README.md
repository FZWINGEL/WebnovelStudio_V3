...
- **Workshop qualification evidence:** hosted run [34127175895](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/34127175895) passed Ubuntu and the Windows Rust/frontend/build jobs plus the existing 52-check native suite; six bounded Workshop native groups passed before a timeout at the adoption **Kind** selector. Later native suites were skipped, so this is not a broad native pass. Native-first rerun [34129236987](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/34129236987) on source `2a97a39` failed in the six bounded Workshop groups at the prefilled **Content** selector; Ubuntu and Windows checks/build passed, later native gates were skipped; the harness role fix is present and a fresh rerun is pending.
- **Latest bounded live Workshop smoke:** source `2a97a39` completed one Luna/xhigh/priority request at 13:47 UTC with 7,289 stdin tokens, 2,106 input tokens, 2,354 output tokens, 1,167 reasoning tokens, and three valid directions. The unchanged anchor, manual path, and no-chapter boundaries held. The run exposed internal anchor and `affectedTargets` metadata in the UI; current/historical flag projection and backend anchor-override handling were then fixed while preserving raw responses and real flags. This is one headless live smoke, not broad provider or quality qualification.
