...
- **Persistence floor:** the current project database reader floor is schema 30, and the library/preferences schema is 4. Schema 27's author-binding boundary remains preserved; schema 28 adds frozen source-title projection validation, schema 29 adds `memory_results.delivery_json`, and schema 30 adds nullable `provider_results.reported_model` while preserving legacy packet bytes, hashes, and absent delivery values.
