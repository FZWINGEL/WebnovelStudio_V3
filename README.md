...
- **Story Workshop (development slice):** Develop/Write workbench with six lenses—Overview, World, People, Themes & tone, Story possibilities, and Notebook—for manual exploration and explicit generation. Requests produce typed candidates and reviewable alternatives; only an explicit preview and adoption changes nonchapter documents. Atomic add/replace adoption supports existing and new relationship endpoints, exact source-head checks, and reviewable impact categories. Author-room decisions and protected details remain separate from writing eligibility, character knowledge, and manuscript evidence. Workshop persistence uses schema 35. See the [Story Workshop implementation ledger](docs/V3_STORY_WORKSHOP_IMPLEMENTATION.md).
- **Workshop qualification evidence:** [CI 34133645198](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/34133645198) passed all 15 Workshop native checks and the existing editor, HTTP, close, interruption, recovery, and memory-lookup suites. The Workshop flow uses the local mock and synthetic projects; author evaluation and broader native/provider coverage remain open. Detailed results are in the [implementation ledger](docs/V3_STORY_WORKSHOP_IMPLEMENTATION.md).
...
- **Current private installer:** [package run 34130743953](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/34130743953) passed the installed-release, synthetic write/reopen, normal close, and same-version uninstall/reinstall retention checks on source `df747b6`. This is a fresh Workshop-era installer, distinct from the historical 3.0.0 build. Artifact identity and remaining release gates are in [Windows package qualification](docs/WINDOWS_PACKAGE_QUALIFICATION.md).
