...
The Story Context Engine retains original evidence, freezes each request's permitted sources, and records the exact input sent to the assistant. You can also review and retain an exact chapter version without running a model. The core freezes an exact reviewed basis consisting of earlier reviewed chapters plus the current working target over IPC. The committed schema-19 story-continuation slice adds explicit Working/Reviewed basis choice, an editable typed paragraph preview, append-only validation, and atomic Apply/Reject; it has 252 frontend tests, 428 Rust tests, one bounded live continuation result, and prior [CI run 34008911179](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/34008911179) passing both contract jobs and all 36 native checks. Broader live-provider and full native/release qualification remain pending. The C4-A Story Memory panel now provides an explicit, source-linked navigation digest for one full current chapter, using the selected local mock or qualified Luna/Max/Fast profile. Current chapter views can be reused as bounded Working discussion context when full prose does not fit, with separate generated coverage and exact evidence; they remain non-authority and are excluded from restricted writing and reviewed continuation. Schema-20 reviewed export is implemented and locally native-qualified as a development slice; its diagnostic made no new LLM calls, while full wrapper/CI requalification remains pending. Accepted story facts, richer C4 digests and lookup, typed reviewed records, C5/C6 work, and full release qualification remain open. See [implementation status](docs/IMPLEMENTATION_STATUS.md) for executed checks and [Codex qualification](docs/CODEX_QUALIFICATION.md) for the precise live-provider boundary.
