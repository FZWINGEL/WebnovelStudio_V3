...
- **Workshop qualification evidence:** [CI 34130744589](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/34130744589) passed Ubuntu and Windows checks/build, then ten of fifteen Workshop native check groups before the harness tried to read a collapsed starting-idea panel after reopening. The harness correction is present; the remaining Workshop scenarios and later native suites need a fresh passing run. Detailed evidence and earlier failures are retained in the [implementation ledger](docs/V3_STORY_WORKSHOP_IMPLEMENTATION.md).
- **Latest bounded live Workshop smoke:** source `2a97a39` completed one Luna/xhigh/priority request at 13:47 UTC with 7,289 confirmed stdin bytes, 2,106 input tokens, 2,354 output tokens, 1,167 reasoning tokens, and three valid directions. The unchanged anchor, manual path, and no-chapter boundaries held. The response named internal anchors as affected material; source review showed that these could become visible review flags. Current/historical flag projection and backend anchor-override handling were then fixed while preserving raw responses and real flags. A separate read-only database reopen verified the saved packet, receipt, response, and usage without another model call. This is one headless live smoke, not broad provider or quality qualification.
