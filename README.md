...
- **Provider and model direction:** the V2 model picker and adapter/catalog behavior are being carried into V3 so author-facing requests can use an explicit model and traits. A configurable OpenAI-compatible endpoint adapter is also planned, with endpoint URL, model, credentials, and capability metadata kept in Settings. All background summary/memory work uses GPT-5.6-Luna/xhigh. Codex uses installed-version compatibility checks and records observed executable identity per request; it is not version-pinned. These provider additions remain in progress.
...
- **Persistence floor:** the current project database reader floor is schema 25. Schema 24 retains the bounded lookup records; schema 25 adds optional observed provider runtime identity while preserving legacy packet bytes and hashes.
