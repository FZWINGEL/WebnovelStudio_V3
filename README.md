...
Chapter continuation offers Working/Reviewed story choices, an editable paragraph preview, and explicit Apply/Reject. Exports offer working drafts or exact author-reviewed snapshots. These are development features; typed accepted story records, richer memory and lookup, broader provider support, and full release qualification remain open. See [implementation status](docs/IMPLEMENTATION_STATUS.md) for executed checks and [Codex qualification](docs/CODEX_QUALIFICATION.md) for the live-provider boundary.
