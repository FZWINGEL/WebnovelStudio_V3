...
**In development.** The persistent workspace runs locally, and discussion currently uses a clearly labelled deterministic test model. The selected-passage proposal/review/Apply slice now also supports an optional author-approved writing brief in local development. The current embedded Tauri build passed a 25-check local native diagnostic on WebView2 `152.0.4191.62`; the diagnostic omits only clipboard from the strict 26-check flow. The core checks and local native diagnostic pass; hosted qualification remains pending. Installed-release qualification and production live AI remain unfinished. The separate native Codex discovery and synthetic experiments are recorded in [Codex qualification](docs/CODEX_QUALIFICATION.md). See [implementation status](docs/IMPLEMENTATION_STATUS.md) for exact test evidence and remaining work.
...
- **This W5 checkpoint:** suggest edits for a selected chapter passage with deterministic review cards, editable previews, Apply, and Reject. The current local native diagnostic covers the writing-brief flow; hosted qualification of this addition remains pending.
