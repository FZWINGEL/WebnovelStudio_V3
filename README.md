...
The Story Context Engine retains original evidence, freezes each request's permitted sources, and records the exact input sent to the assistant. You can also review and retain an exact chapter version without running a model. The core freezes an exact reviewed basis consisting of earlier reviewed chapters plus the current working target over IPC. The current story-continuation slice adds explicit Working/Reviewed basis choice, an editable typed paragraph preview, append-only validation, and atomic Apply/Reject; one bounded live continuation request is recorded and CI passes both contract jobs and all 36 native checks. Broader live-provider and full native/release qualification remain pending. The C4-A Story Memory panel now provides an explicit, source-linked navigation digest for one full current chapter, using the selected local mock or qualified Luna/Max/Fast profile. Current chapter views can be reused as bounded Working discussion context when full prose does not fit, with separate generated coverage and exact evidence; they remain non-authority and are excluded from restricted writing and reviewed continuation. Accepted story facts, richer C4 digests and lookup, typed reviewed records, C5/C6 work, and full release qualification remain open. See [implementation status](docs/IMPLEMENTATION_STATUS.md) for executed checks and [Codex qualification](docs/CODEX_QUALIFICATION.md) for the precise live-provider boundary.
...
- **Reviewed context (F2-B core slice):** freeze the exact earlier reviewed prefix and current working target through the Rust/IPC boundary, with immutable reader-position pins and historical namespace fencing. The story-continuation slice consumes this boundary for its explicit Reviewed basis; live dispatch and ready export remain open. See the [reviewed context contract](docs/ADR_0013_REVIEWED_CONTEXT.md).
- **Story continuation (current local development slice):** choose Working draft or Reviewed story in the chapter assistant, receive one typed append-only candidate, edit its paragraphs, and Apply or Reject it without rewriting existing blocks. Schema 19 persists the continuation kind and prepared paragraphs; retries retain the selected basis, operation identity, generated IDs, and exact body after an uncertain acknowledgment. The deterministic local check and local WebView2 diagnostic pass (35 of 36 checks, with only the known clipboard check omitted), and one bounded live request returned valid paragraphs that were applied and reopened without another request. Strict CI, broader live-provider, and full native/release qualification remain pending. See the [story continuation contract](docs/ADR_0016_STORY_CONTINUATION.md).
