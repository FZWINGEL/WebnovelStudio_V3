...
describe('persistent FeedbackPanel safeguards', () => {
  ...
  // The selection omits `reasoning`/`serviceTier` when unset; the binding always
  // carries them, and Rust reads absence as `None`, so `null` says the same thing.
  const httpBinding: ProviderBinding = { ...httpSelection, reasoning: httpSelection.reasoning ?? null, serviceTier: httpSelection.serviceTier ?? null, profileVersion: 'openai-chat-completions.v1', inputLimitBytes: '24576', reservedOutputBytes: '4096', reservedProtocolBytes: '1024', outputLimitBytes: '65536', accountingMethod: 'utf8-byte-count/http-request-v1', http: { baseUrl: 'https://example.test/v1', configRevision: '4', stream: true, responseFormat: 'text' } };
