...
function key(body: ComposerBody): string { return JSON.stringify({ text: body.text, scope: body.scope, pinnedDocumentIds: body.pinnedDocumentIds }); }
...
export class ComposerSession {
  ...
  constructor(private documentId: string, draft: DiscussionDraft | null, private access: () => ProjectAccess, private write: (request: SaveDiscussionDraft) => Promise<DiscussionDraft>) {
    this.body = structuredClone(draft ? { text: draft.text, scope: draft.scope, pinnedDocumentIds: draft.pinnedDocumentIds } : emptyComposer());
