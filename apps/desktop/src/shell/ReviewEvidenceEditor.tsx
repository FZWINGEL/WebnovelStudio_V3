...
import type { EvidenceAnchor, PossessionRecord, StoryEntityRef } from '../ipc/reviews';
...
export function ReviewEvidenceEditor({ records, disabled, captureSelection, onChange, onEditingChange }: {
  ...
  const labels = useMemo(() => {
    ...
    return new Map(entities.map(entity => {
      const index = (seenLabels.get(entity.label) ?? 0) + 1;
      seenLabels.set(entity.label, index);
      return [entity.id, counts.get(entity.label)! > 1 ? `${entity.label} (${index})` : entity.label] as const;
    }));
  }, [entities]);
  ...
  function begin(scope: Scope | null, record: PossessionRecord | null = null) {
    ...
    const objectId = record?.object.id ?? entities[0]?.id ?? '__new_object__';
    setError('');
