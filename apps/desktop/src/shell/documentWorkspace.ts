...
import { useChatFlows } from './chatFlows';
...
export type ActiveDocument = Readonly<{ record: DocumentRecord; session: DocumentSession; viewState: ViewState | null }>;
...
export function useDocumentWorkspace(deps: DocumentWorkspaceDeps) {
  ...
  const {
    chatAdoptionTarget,
    openChatChapterResult, prepareChatAdoption, acceptChatDocuments, restoreChatAdoption,
    createChatNote, createChatChapter, prepareChatChapter, prepareChatSource,
  } = useChatFlows({
