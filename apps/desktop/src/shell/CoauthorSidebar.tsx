...
export function CoauthorSidebar({ current, recent, busy, counts, onLibrary, onNewProject, onOpen, onMaterial, creationForm }: {
  ...
  return <aside className="coauthor-sidebar" aria-label="Story navigation">
    ...
    <div id="coauthor-navigation" className={`coauthor-sidebar-body${expanded ? ' is-expanded' : ''}`}>
      <button className="coauthor-library" disabled={busy} onClick={onLibrary}><BooksIcon aria-hidden size={20} />Your library</button>
      ...
      <nav className="coauthor-projects" aria-label="Your projects">
        ...
        {others.map(item => <button className="coauthor-project" key={item.projectId} disabled={busy || item.missing} onClick={() => onOpen(item.path)}><BookOpenIcon aria-hidden size={21} /><span><strong>{item.title}</strong>{item.missing ? <small>Folder unavailable</small> : <>{item.activityLabel && <small>{item.activityLabel}</small>}{!!item.pendingDrafts && <small>{item.pendingDrafts} draft{item.pendingDrafts === 1 ? '' : 's'} to review</small>}</>}</span></button>)}
      </nav>
      <nav className="coauthor-materials" aria-label="Story material">
        ...
        {[...PROJECT_TABS].sort((a, b) => ['worldbuilding', 'characters', 'chapters', 'plot', 'notes'].indexOf(a.id) - ['worldbuilding', 'characters', 'chapters', 'plot', 'notes'].indexOf(b.id)).map(tab => {
          ...
          return <button key={tab.id} disabled={busy} onClick={() => onMaterial(tab.id)}><Icon aria-hidden size={19} /><span>{tab.label}</span><small aria-label={`${counts[tab.id]} documents`}>{counts[tab.id] || ''}</small></button>;
        })}
      </nav>
