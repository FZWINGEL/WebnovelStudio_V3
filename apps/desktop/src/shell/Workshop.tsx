...
export const Workshop = forwardRef<WorkshopHandle, Props>(function Workshop({ project, navigationBusy = false, onOpenDocument, onDocumentsChanged, onError }, ref) {
  ...
  async function exploreRelationship(relationship: WorkshopRelationship) {
    ...
    try {
      ...
      const next = { ...newSession('people'), title: direction.slice(0, 160), workingTitle: direction.slice(0, 160),
        ...
        composer: 'Explore this relationship. Distinguish what is established, what is uncertain, and what might change. Keep unrelated biography and world history unchanged.' };
      if (!canChangeExploration()) return;
  ...
  function differentQuestion() {
    if (!session) return;
    const choices = [...WORLD_QUESTIONS, ...LENSES.map(lens => ({ text: lens.question, reason: lens.reason }))];
    const start = choices.findIndex(question => question.text === session.focusQuestion);
    const next = Array.from({ length: choices.length }, (_, offset) => choices[(start + offset + 1) % choices.length]).find(question => question.text !== session.focusQuestion && !session.questions.some(record => record.text === question.text && record.status !== 'open'));
