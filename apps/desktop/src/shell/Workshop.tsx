...
import { CandidateBoard } from '../workshop';
...
import { RequestContext } from '../workshop';
import { NextExplorationContext } from '../workshop';
import { BranchComparison } from '../workshop';
...
export const Workshop = forwardRef<WorkshopHandle, Props>(function Workshop({ project, navigationBusy = false, onOpenDocument, onDocumentsChanged, onError }, ref) {
  ...
  async function bringDocument(documentId: string) {
    ...
    try {
      ...
      edit(current => ({ ...current, relationshipId: undefined, focusDocumentId: documentId, title: source.title, workingTitle: source.title, originalNotes: plainText(source.body), brief: plainText(source.body), selectedScope: `Element: ${source.title}`, includedDocumentIds: [...new Set([...current.includedDocumentIds, documentId])] }), true);
      setCapture(null);
  ...
  function chooseQuestion(text: string, reason: string, lens?: WorkshopLens, explicitlyChosen = false) {
    if (!session) return;
    const disposition = session.questions.find(question => question.text === text && question.status !== 'open');
    if (disposition && !explicitlyChosen) {
    ...
    edit(current => ({ ...current, ...(lens ? { lens } : {}), focusQuestion: text, focusReason: reason,
      questions: explicitlyChosen ? current.questions.map(question => question.text === text ? { ...question, status: 'open' as const } : question) : current.questions }));
    setNotice('');
  }
