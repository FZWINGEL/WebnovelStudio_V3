...
**Status:** C0 contracts, C1 working-basis evidence/retrieval, C2 frozen packets, parts of C3 guidance/conversation/source integration, and C4-A explicit chapter navigation memory are implemented. C4-B automatic generated-view reuse in working author-room discussions has local contract and strict native CI evidence. C4-C exact chapter freshness has integrated local and strict native CI evidence. Single-passage and structured Apply, history, schema-20 export, optional approved writing briefs, author-only exact chapter review, schema-19 continuation, schema-21 reviewed-story records, schema-23 promise history, and C5-A evidence history are wired into the native app at documented development boundaries. Promise history is an implemented development boundary; current native, bounded live, and CI evidence is maintained in [implementation status](IMPLEMENTATION_STATUS.md). The C6 bounded lookup implementation is in progress: focused protocol, packet, boundary, core, frontend, and local native checks exist for the opt-in Working/AuthorRoom/Discuss search/read route, and the accepted wrapper checkpoint passes 493 core tests, 29 desktop tests, one ignored fixture, and 324 frontend tests in 26 files. One accepted-build native live lookup and same-data reopen now have bounded evidence; hosted C6 and broader live qualification remain open. See implementation status. Broader live coverage, author-trial, and full release qualification remain open. The bounded Codex path uses the exact compiled packet with explicit application byte caps and durable delivery/usage outcomes; broader provider qualification remains open. Richer relevance selection, higher-level C4 digests, broader C5 state views, restricted-writing lookup, manual rebind, and batch Apply remain open.

The supplied research and proposal are rationale for this adopted design, not newly verified validation or completed implementation evidence. Background summaries, chapter-memory refreshes, and other maintenance analyses use GPT-5.6 Luna with xhigh reasoning. Author-facing writing remains governed by the selected V2-style model picker. Provider adapters, including configurable OpenAI-compatible endpoint APIs, are a separate in-progress integration and do not change the context authority contract.

All background summary, story-memory, and maintenance LLM calls use GPT-5.6
Luna with xhigh reasoning. Author-facing writing uses the persistent V2-style
model picker. The provider adapters and configurable OpenAI-compatible endpoint
APIs needed to enforce these routes remain in progress.
...
Reuse reviewed story records. The schema-23 promise-history development slice now records explicit setup, payoff, cancellation, or unclear observations against opaque promise identities, supports reuse across reviewed chapters, and queries exact authenticated history with incomplete-evidence wording. Restricted packets filter author-room promise records before delivery; source prose remains governed independently by the request policy. Current native, bounded live, and CI evidence is maintained in [implementation status](IMPLEMENTATION_STATUS.md). Broader live coverage, author-trial, and remaining C5 state views remain open. Remaining C5 work includes last established possession plus known transitions, character belief/knowledge at a boundary, broader thread/state views, and explicit creative decisions for a scope.
...
The lookup records were introduced in schema 24. The current reader floor is
schema 34, including the typed reviewed-memory lookup boundary in
[ADR 0032](ADR_0032_STORY_MEMORY_LOOKUPS.md). Legacy capabilities remain absent
and historical packets remain byte/hash compatible.

The first C6 implementation slice is an opt-in route for Working, AuthorRoom, and Discuss conversations. It uses the strict application response envelope `story-lookup.v1`: a model may request bounded local search or exact source reads, and Rust resolves them against one frozen project snapshot. The initial invocation plus at most two fresh invocations are allowed. Each child packet, read, response, and final answer is durably recorded; schema 24 raises the reader floor for the new invocation/read records. The composer choice is off by default, persists with the request identity, clears when switching to Suggest edits or Continue, and is inspectable per packet. Intermediate protocol JSON remains evidence, not chat text.

This route is application-controlled and does not claim provider-native function calling. The schema-34 extension adds exact reviewed identity discovery and paged knowledge, possession and promise histories, with the same frozen source and allowance. It does not expose restricted-writing lookup, filesystem/session access, automatic canon, or manuscript edits. Application byte allowances remain distinct from model token limits and billing. The ordinary one-packet discussion route remains available with lookup disabled.

**Historical first-lookup evidence (current results are in implementation status):** the pure lookup protocol has seven focused tests, the packet compiler has 34 focused tests, the independent boundary suite has nine tests, the core lookup suite has four tests, the native worker has four tests, and the frontend suite has 324 tests in 26 files. The accepted C6 wrapper checkpoint passes 493 core tests, 29 desktop tests, one ignored fixture, and 324 frontend tests in 26 files. Review fixes cover crash/reopen outcomes, exact child-run provenance, terminal/result/assistant consistency, historical unavailable gaps, reproducible search metadata, and truthful delivery status. The focused native lookup journey passed with no page errors, and the wider local native diagnostic passed 45/46 checks with the known OS clipboard check excluded. Native accepted-build, hosted C6, live-provider lookup, narrative evaluation, and release qualification remain open.

**Completion:** Stop prevents later authorized calls once committed; duplicate tool events do not duplicate local reads or silently cause paid retries; budget exhaustion is visible; a broken stream preserves incomplete output; a crash at dispatch remains an explicit unknown external outcome. A fresh invocation after repacking is not mislabeled as provider-session resume. Restricted-writing lookup and model-specific token accounting require separate qualification.
