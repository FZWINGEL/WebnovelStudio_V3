...
The remaining C3 work is linked retry guidance reuse, persistent chapter/project source pins, and safe-brief transfer. Current one-use guidance applies to a new logical request only; a linked retry does not yet reuse consumed request guidance. The updated native smoke checkpoint passes 18/18 locally and remains development evidence rather than final release qualification.
