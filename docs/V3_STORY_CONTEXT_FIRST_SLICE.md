...
**Status:** C0 contracts, C1 working-basis evidence/retrieval, C2 frozen packets, and parts of C3 guidance/conversation/source integration are implemented. Single-passage Apply, history, export, and optional approved writing briefs are wired into the native app. The bounded Codex path uses the exact compiled packet with explicit application byte caps and durable delivery/usage outcomes; full live-provider qualification remains open. The committed baseline `be1d93c` passed all 28 strict native checks in CI 33994609086; that evidence predates live-provider/import integration. Current evidence is maintained in [implementation status](IMPLEMENTATION_STATUS.md). Richer relevance selection, C4–C6, reviewed story, whole-chapter/block/manual-rebind, and batch Apply remain open.
