...
**Status:** C0 contracts, C1 working-basis evidence/retrieval, C2 frozen packets, and parts of C3 guidance/conversation/source integration are implemented. Single-passage Apply, history, export, optional approved writing briefs, and author-only exact chapter review are wired into the native app. Review records do not yet enable reviewed context or generation. The bounded Codex path uses the exact compiled packet with explicit application byte caps and durable delivery/usage outcomes; full live-provider qualification remains open. Current evidence is maintained in [implementation status](IMPLEMENTATION_STATUS.md). Richer relevance selection, C4–C6, typed accepted story records, the reviewed resolver, whole-chapter/block/manual-rebind, and batch Apply remain open.
