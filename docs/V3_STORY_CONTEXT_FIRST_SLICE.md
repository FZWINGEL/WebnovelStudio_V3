...
**Status:** C0 contracts, C1 working-basis evidence/retrieval, C2 frozen packets, parts of C3 guidance/conversation/source integration, and C4-A explicit chapter navigation memory are implemented. Single-passage Apply, history, export, optional approved writing briefs, and author-only exact chapter review are wired into the native app. F2-B adds the reviewed basis manifest, immutable reader-position pins, and an explicit core/IPC freeze of the exact earlier reviewed prefix plus current working target. This does not add continuation UI, live dispatch, ready export, or typed reviewed-story records. The bounded Codex path uses the exact compiled packet with explicit application byte caps and durable delivery/usage outcomes; full live-provider qualification remains open. Current evidence is maintained in [implementation status](IMPLEMENTATION_STATUS.md). Richer relevance selection, C4 derived-view packet reuse, C5–C6, whole-chapter/block/manual-rebind, and batch Apply remain open.
