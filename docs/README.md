...
Start with the product requirements and [implementation status](IMPLEMENTATION_STATUS.md). The native development app has persistent writing, project management, discussion, exact context inspection, adopted guidance, saved sources, optional approved writing briefs, selected-passage Apply/Reject, history, export previews, bounded Codex integration, author-only exact chapter review, the F2-B core freeze of reviewed earlier chapters plus a current working target, explicit chapter memory with automatic reuse of current generated views in working discussions, the committed schema-19 story-continuation development slice, and schema-8 V2 import. The continuation slice has 252 frontend tests and 428 Rust tests, a local diagnostic passing 35 of 36 checks with the known clipboard check omitted, and one bounded live continuation request that completed and reopened successfully; [CI run 34008911179](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/34008911179) remains the latest confirmed continuation CI and passed both contract jobs and all 36 native checks. Schema-20 reviewed export is implemented and locally native-qualified as a development slice; full wrapper/CI requalification remains pending. Broader live-provider and full native/release qualification, typed reviewed-story authority, richer memory, and broader providers remain open. Design documents describe the target; source and executed checks establish current behavior.
...
| 6r | [Reviewed export ADR](ADR_0017_REVIEWED_EXPORT.md) | Schema-20 reviewed export locally native-qualified as a development slice: exact author-reviewed chapter export, freshness acceptance, and historical file receipts; full wrapper/CI requalification pending |
