...
Start with the product requirements and [implementation status](IMPLEMENTATION_STATUS.md). The native development app provides persistent writing and project management, discussion and context inspection, adopted guidance and approved briefs, selected-passage and structured block Apply/Reject, chapter continuation, saved versions, working/author-reviewed export, bounded Codex integration, explicit chapter memory with reuse in working discussions, schema-21 passage-backed reviewed details, schema-23 promise history, partial C5-A entity reuse and evidence history, and an in-progress opt-in C6 story lookup route. Promise history is an implemented development slice with native and bounded live evidence; C6 has focused protocol, packet, boundary, core, frontend, and local native development checks. Current verified checks total 569 active Rust tests (529 core and 40 desktop) plus 343 frontend tests in 27 files; updated hosted checks and broader live qualification remain open. The native HTTP fixture and 45/46 local regression diagnostic pass; the known local clipboard case remains covered by strict CI. See [implementation status](IMPLEMENTATION_STATUS.md). The current provider surface compatibility-checks the installed Codex CLI without version/hash pinning, routes background summary/memory jobs to GPT-5.6 Luna/xhigh, carries the V2 model picker rail/search/favorites/keyboard/traits, and provides configurable OpenAI-compatible endpoint profiles with a native HTTP worker. HTTP lookup, V2 CLI adapter parity, dynamic Codex model discovery, broader live-provider support, and full release qualification remain open. [CI 34017484597](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/34017484597) remains an earlier hosted checkpoint. Design documents describe the target; source and executed checks establish current behavior.
...
| 6w | [Bounded story lookups ADR](ADR_0022_BOUNDED_STORY_LOOKUPS.md) | C6 implementation in progress: opt-in `story-lookup.v1` search/read route for Working, AuthorRoom, and Discuss; schema-24 durable invocation/read records under the current schema-26 project reader floor, an initial call plus at most two further calls, and per-packet inspection; focused/local native evidence and the accepted wrapper checkpoint exist, while native accepted-build, hosted, and live-provider qualification remain pending |
| 6x | [OpenAI-compatible endpoints ADR](ADR_0023_OPENAI_COMPATIBLE.md) | Configurable endpoint profiles, native credential readiness, model catalog/manual entries, V2-style provider selection rails, and the bounded native HTTP transport; HTTP lookup, V2 CLI adapter parity, dynamic Codex discovery, and native/live qualification remain open |
