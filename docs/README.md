...
| 6w | [Bounded story lookups ADR](ADR_0022_BOUNDED_STORY_LOOKUPS.md) | C6 implementation in progress: opt-in `story-lookup.v1` search/read route for Working, AuthorRoom, and Discuss; schema-24 durable invocation/read records, an initial call plus at most two further calls, and per-packet inspection; focused/local native evidence and the accepted wrapper checkpoint exist, while native accepted-build, hosted, and live-provider qualification remain pending |
