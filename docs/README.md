...
Start with the product requirements and [implementation status](IMPLEMENTATION_STATUS.md). The default Tauri application has a persistent Library/Workspace, manual writing, mock discussions, scoped guidance, and an inspectable context packet. Durable Apply, reviewed-story authority, richer memory, live providers, and release qualification remain open. Design documents describe the target; source and executed checks establish current behavior.
