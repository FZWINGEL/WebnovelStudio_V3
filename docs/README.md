...
Start with the product requirements and [implementation status](IMPLEMENTATION_STATUS.md). The native development app provides persistent writing and project management, discussion and context inspection, adopted guidance and approved briefs, selected-passage Apply/Reject, chapter continuation, saved versions, working/author-reviewed export, bounded Codex integration, explicit chapter memory with reuse in working discussions, schema-21 passage-backed reviewed details, and independent V2 schema-8 import. [CI 34010306332](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/34010306332) remains the reviewed-export checkpoint; schema 21 passes the final 39/40 local native diagnostic, omitting only the known local OS clipboard case, while hosted CI qualification is pending. Broader C5 state views/retrieval, providers, author trials, and full release qualification remain open. Design documents describe the target; source and executed checks establish current behavior.
...
| 6s | [Reviewed story evidence ADR](ADR_0018_REVIEWED_STORY_EVIDENCE.md) | Schema-21 implementation with final 39/40 local native diagnostic: exact passage-backed possession records, immutable review sets, audience-filtered context delivery, and distinct evidence inspection; hosted CI pending |
