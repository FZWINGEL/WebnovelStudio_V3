...
Start with the product requirements and [implementation status](IMPLEMENTATION_STATUS.md). The native development app has persistent writing, project management, discussion, exact context inspection, adopted guidance, saved sources, optional approved writing briefs, selected-passage Apply/Reject, history, export previews, bounded Codex integration, author-only exact chapter review, the F2-B core freeze of reviewed earlier chapters plus a current working target, explicit chapter memory with automatic reuse of current generated views in working discussions, the local story-continuation development slice, and schema-8 V2 import. The local continuation diagnostic passes 35 of 36 checks with the known clipboard check omitted, and one bounded live continuation request completed and reopened successfully; CI passes both contract jobs and all 36 native checks; broader live-provider and full native/release qualification, ready export, typed reviewed-story authority, richer memory, and broader providers remain open. Design documents describe the target; source and executed checks establish current behavior.
...
| 6r | [Reviewed export ADR](ADR_0017_REVIEWED_EXPORT.md) | Next unbuilt package: exact author-reviewed chapter export, freshness acceptance, and historical file receipts |
