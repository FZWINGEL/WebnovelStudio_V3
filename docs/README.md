...
Start with the product requirements and [implementation status](IMPLEMENTATION_STATUS.md). The native development app provides persistent writing and project management, discussion and context inspection, adopted guidance and approved briefs, selected-passage and structured block Apply/Reject, chapter continuation, saved versions, working/author-reviewed export, bounded Codex integration, explicit chapter memory with reuse in working discussions, schema-21 passage-backed reviewed details, partial C5-A entity reuse and evidence history, and independent V2 schema-8 import. The current structured-suggestion wrapper passes 467 active Rust tests and 301 frontend tests; the local native diagnostic passes 42/43 checks, omitting only the known local OS clipboard case. Hosted CI for this current slice remains pending. The prior C5-A checkpoint is recorded in [implementation status](IMPLEMENTATION_STATUS.md). Broader C5 state views/retrieval, providers, author trials, and full release qualification remain open. Design documents describe the target; source and executed checks establish current behavior.
...
| 6u | [Structured suggestions ADR](ADR_0020_STRUCTURED_SUGGESTIONS.md) | Local development slice for explicit block-range and whole-chapter suggestions: typed rich blocks, editor-owned IDs, protected surroundings, and atomic Apply; local native 42/43 with the known clipboard case omitted, hosted CI pending |
