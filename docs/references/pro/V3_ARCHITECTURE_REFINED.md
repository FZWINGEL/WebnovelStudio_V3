...
**Status:** proposed implementation contract, not an implemented or tested application.  
**Companion:** [First-slice plan](V3_FIRST_SLICE_PLAN.md).  
...
2. **Saving and applying have different reconciliation rules.** An autosave acknowledgment only advances a saved-generation watermark. Applying an accepted suggestion uses a short, local editing barrier, an exact-version precondition, a durable transaction, and then a preflighted editor transaction. No save acknowledgment calls `setContent` on newer text.
...
All requested files were present and readable in `CHATGPT_PRO_CONTEXT.zip`. I inspected the proposal, `PRODUCT.md`, the stabilization update and historical findings in `CURRENT_STATE_REVIEW.md`, relevant sections of both research reports, the optional production kit, the repository evidence ledger, and the supplied screenshot evidence and README. The screenshots document V2; they are not V3 designs or native-editor tests.

The archive **does not contain the V2 application source tree, actual database/schema migrations, a representative manuscript database, or executable test fixtures**. Relative links to `src/...` in the proposal do not supply those files. I did not run the reported V2 tests, launch the application, test a provider, or qualify Windows IME behavior. Public dependency documentation and selected dependency source were inspected separately.
...
| **Verified technical fact `[S…]`** | Official documentation or primary source inspected for this review. Dates/versions and limitations appear in §19. |
...
Unless expressly attributed, proposed schemas, policies, UX labels, thresholds, and algorithms below are architectural decisions. They are not benchmark results or research-derived universal rules.
...
| Applying an edit | Flush, short local mutation barrier, Rust commit, exact JS transaction | Optimistic mutation followed by rollback/rebase | Simplest honest cross-process acknowledgment semantics. Briefly suspends document-changing input, never a provider-length wait. | Design; experiment E2 in companion plan | The barrier is perceptibly disruptive on supported hardware, or concurrent editing becomes a real requirement. |
...
Conversation is not a background critique trigger. The author submits “The second half feels rushed; make the confrontation more emotional but keep the ending.” The assistant may discuss a plan. **Propose edits** produces review cards; **Apply** changes prose. There is no automatic paid critique when a chapter is created, saved, or marked ready.
...
| Discuss / Propose edits | Conversation-only request / proposal-producing request; neither mutates prose |
...
flowchart TB
  ...
  Editor --> Sync[DocumentSession<br/>captured autosaves and apply barrier]
...
The following illustrates the protocol, not a generated client implementation. All IDs are opaque. Versions are validated decimal strings on the wire to avoid JS integer precision assumptions. Volatile session/lease fields are excluded from operation payload hashes so an identical operation can be recovered under a newly issued lease.

```typescript
type Id = string;
type Version = string;
type Hash = string;
type ProjectAccess = { projectId: Id; session: Id; writerLease: Id };
...
type SaveAck = {
  operationId: Id; head: Head; savedGeneration: Version;
};
...
type CancelJob = { projectId: Id; session: Id; operationId: Id; runId: Id };
type Reconcile = {
  projectId: Id; session: Id; documentId: Id;
  pendingOperationIds: Id[]; observedJobSequences: Record<Id, Version>;
};
```

`PrepareProposal.replacement` is validated against the restricted fragment grammar, not accepted as arbitrary executable JSON. Production DTOs use a tagged fragment type. Rust derives the base hash, grant containment, result hash, and application target from saved data. The frontend cannot turn a context reference into a wider grant by changing a field.
...
BEGIN IMMEDIATE
  ...
  insert operation receipt(result head, generation, payload hash)
COMMIT
return SaveAck
```
...
1. **Acquire a short mutation barrier in the document session.** Wait for composition to end. Disable document-changing input, paste/drop, undo/redo, and further Apply actions before flushing. Keep selection visible and announce “Applying change.” Do not simulate or buffer arbitrary IME events. An author actively composing can finish before applying.
2. **Drain any in-flight save and flush the newest captured body.** Since the barrier was acquired first, no newer local body can appear between the flush and commit. A save failure aborts the application without changing the editor.
...
6. **On definite success, dispatch the preflighted transaction to the same editor view.** Map selection; isolate the history event; update the saved watermark to the committed result; suppress a redundant autosave of that exact durable transaction. Release the barrier. No newer local changes existed to overwrite.
7. **On unknown outcome, reconcile before editing resumes.** Rotate/fence the old writer lease on the project thread, then read the receipt and latest head. A late old command will fail its old lease. A committed operation is not repeated; an absent operation can be resubmitted with the same logical payload/operation ID under the new lease.

Only local validation and persistence occur behind the barrier. Never hold it for a provider request, a continuity review, or a backup. If a database write is slow, show the unfinished local operation honestly. An unexpected post-commit editor mismatch must not roll back the database or reapply against a different body: freeze that editor, retain diagnostics and any local buffer, and reload the committed state through an explicit recovery path. This exceptional path may reset undo history; the normal path does not remount the editor.

The barrier is a deliberate simplification. Its latency and IME/focus behavior are release-gating experiments. “Autosave runs asynchronously” alone is not a solution to this cross-process race.
...
BEGIN IMMEDIATE
  receipt hit -> same payload? return its recorded outcome
...
|---|---|
| `project` | One row: stable project ID, title, format version, metadata version, story/order epochs. ID is not a folder name. |
...
| `command_receipts` | Unique operation ID, logical payload hash, small result/reference data. Stored in the same transaction as the effect. |
...
CREATE TABLE revisions (
  ...
  parent_id TEXT REFERENCES revisions(id), reason TEXT NOT NULL,
  UNIQUE(document_id, source_working_version), UNIQUE(document_id, id)
);
CREATE TABLE command_receipts (
  operation_id TEXT PRIMARY KEY NOT NULL, payload_hash TEXT NOT NULL,
  operation_kind TEXT NOT NULL, result_json TEXT NOT NULL
);
CREATE TABLE decisions (
  ...
  operation_id TEXT NOT NULL REFERENCES command_receipts(operation_id),
...
This omits auxiliary tables and deferred circular pointer constraints for readability. Actual migrations must add same-document composite foreign keys for ready/checkpoint pointers and source targets, uniqueness for `(thread_id, message_sequence)` and `(suggestion_id, version)`, and checks that applied decisions have before/after references while rejected decisions do not claim an application. Insert the receipt before dependent decisions inside the same transaction or make that FK deferred. Immutable tables have no ordinary update/delete API; corruption/invariant checks verify pointer ownership and hashes on import and recovery.
...
Keep command receipts small: hashes, identifiers, outcomes, and head metadata—not another copy of each save payload. Initially retain them; avoid pruning idempotency keys while recovery is still being qualified. A future retention policy must explicitly retire request namespaces, not permit an old operation ID to execute again after its receipt disappears.
...
Duplicate uses a consistent project backup, gives the copy a new project ID, preserves document IDs within that new namespace, and retains history/discussions. Runtime jobs become historical/interrupted imports, not runnable work. Copied command receipts retain provenance only; they are not executable receipts in the new operation namespace. Credentials and provider sessions are not copied. The UI explains whether a selected model needs setup on this machine.

### 8.2 Project switching and normal close

Switching is a **detach-after-flush** operation. Wait for composition to commit, acquire the short navigation barrier, drain saves, persist composer/caret/thread, and checkpoint the document. Only then dispose the editor and activate the destination. The captured project/document identity travels with every command. A later event for A cannot save into B.
...
Normal app close flushes all unsaved material first, then offers to stop running replies or remain open. Once stopping is chosen, persist stop intents, terminate local process trees/streams, join local tasks or fence their callbacks as interrupted, and close connections. A forced OS shutdown/crash cannot promise to preserve text that was never committed. No next-launch automatic model retry is allowed.
...
| Passage proposal / whole-chapter rewrite | Exact target, safe author-approved instruction, scope grant, applicable constraints, read-only neighboring prose | Relevant prior prose, permitted terminology, scoped promises | Prose-producing context uses the selected audience/knowledge boundary, not the raw author-room transcript |
...
The chapter conversation is an author room. It can discuss “The mentor stole the key; do not reveal this until chapter 20.” A later prose proposal must not automatically replay that transcript to the writing request. Instead the UI obtains an author-approved safe brief, for example “Have the mentor hesitate before answering; the POV character interprets it as concern.” Only the permitted brief and selected safe sources enter the proposal-producing call. The author may write this brief manually; sanitizing it is not an automatic extra paid call.

A feedback discussion may produce a revision plan, but its privileged prose examples are not immediately executable suggestions. **Propose edits** assembles a separate scoped writing request using the approved brief. Explicitly moving privileged material into a draft is an author decision, not an accidental consequence of conversation retention.
...
| 3 | Author writes an unsent question in `t-A` and chooses New project B → Blank chapter. | The switch barrier drains A1, sends `save-A2(expected=v1,g2)`, commits v2, persists composer/caret/thread, and checkpoints `r-A2`. Only then detach A and finish B. | On A2 disk failure, stay on A with g2 and the composer retained. Do not create a misleading empty replacement of A. |
...
| 2 | Assistant discusses a plan. Author selects Propose edits and approves a writing brief. The ending is outside the three proposed ranges. | `StartFeedback(edits-7)` freezes r7-30 with a safe prose-producing context and a chapter-level proposal limit. | Plan history remains in the author room; only the approved brief/allowed sources enter the writing request. |
...
sequenceDiagram
  ...
  E->>R: New renderer: ReconcileAndFence(K75)
  R->>R: Revoke old writer lease
  R->>D: Read latest working head + K75 receipt + decisions
  D-->>R: v61, K75 already applied
  R-->>E: New lease; current v61; recorded decision
  E->>R: Retry same logical K75, if still requested
  R-->>E: Recorded outcome; no second application
```

All earlier manual typing was flushed before K75. There cannot be newly accepted editor input between its commit and acknowledgment in the failed renderer: the mutation barrier was still held. This is the reason for the barrier, not an assumption that SQLite controls JavaScript memory.
...
P0 blocks any real-manuscript trial; P1 blocks a release claiming the associated feature; P2 informs later quality/performance work. These are **tests to implement**, not tests executed in this review.
...
The actual V2 schema is missing from the archive, so the importer cannot honestly be specified as a list of verified V2 table/column mappings. Minimum additional evidence for implementation is **the V2 migrations/current schema, its backup/export and approval-pointer contracts, and sanitized representative snapshots**, including working-newer-than-approved text and unresolved continuity. The architecture does not require the full application tree merely to start editing safely in V3.
...
| Existing V2 manuscripts can be mapped without data loss | Obtain the missing schema/contracts and representative source snapshot; require a reconciliation report before retirement |
