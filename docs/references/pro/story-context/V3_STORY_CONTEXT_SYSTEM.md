...
**Status:** proposed architecture; no application implementation or benchmark execution is claimed.  
...
`reserved_generation` includes reasoning according to the adapter's actual accounting; do not subtract it twice when it is already included in the output allowance. Tool results and repeated turns consume the same context constraints, not free extra capacity. Count the exact serialized request when possible; label conservative estimates when an exact tokenizer or CLI wrapper accounting is unavailable. Do not use an English words-to-tokens ratio for Chinese text.
...
3. **Local text search:** lexical search for prose plus literal/substring search for Chinese names, quotes, and terminology.
...
SQLite FTS5 provides a suitable local starting point. Its trigram tokenizer's full-text queries do not match substrings shorter than three Unicode characters, so two-character Chinese names need explicit alias/literal lookup. A parameterized `instr(text, query) > 0` scan over the eligible source set is a simple fallback; do not confuse an FTS limitation with absence from the manuscript. [S6]
...
Add an embedding index when a labeled test set shows that names, aliases, links, lexical search, and expansion miss important paraphrased evidence. Compare retrieval improvement at matched packet sizes and evaluate Chinese/English and cross-language queries separately. Vector similarity does not imply entity identity, factual validity, currentness, or permission.
...
A persistent conversation retains all messages locally. Each new request compiles relevant permitted recent turns plus explicitly adopted creative decisions and source-bound references. It does not blindly replay every old response.

A message such as “Keep the ending, remove the sarcastic tone, and don't kill the sister” matters. Offer **Keep as guidance** with a scope: request, chapter, or project. The confirmed instruction becomes a versioned author decision, not merely a sentence inside a lossy chat summary. It is not automatically a world fact.

Conversation digests are generated navigation aids with message-range provenance. If a digest says the author selected option B, the original decision/message must support it. Unaccepted assistant inventions do not become story knowledge through chat compaction. Superseded preferences remain historical; their current scope/version decides whether they are included.
...
| `entity_aliases` | Entity, alias, language/search key, source and acceptance status. Ambiguous matches are allowed; alias text is not globally unique. |
...
| C3 | Mandatory target, instruction, scope, and constraints are never silently truncated. | Tiny budgets, unusually long Chinese text, oversized pinned note, wrapper overhead. |
...
| C10 | Read breadth never enlarges edit scope. | Whole-book context for a repeated Chinese/emoji sentence; only selected occurrence may change. |
...
Create a small author-checked English/Chinese fixture set before selecting embeddings or complex ranking. Include exact and paraphrased retrieval, multi-hop possession changes, old unresolved promises, flashbacks, repeated names, alias ambiguity, false beliefs, early revisions, chat decisions, unavailable evidence, and secret contamination through summaries.
...
| Lexical/link retrieval versus embeddings | Local exact/alias/FTS first | Material gold-evidence recall failures on paraphrase/cross-language cases, fixed by a qualified embedding route. |
