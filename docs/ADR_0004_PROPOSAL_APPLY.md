...
Whole-chapter, block, batch, and manual-rebind proposal Apply contracts remain open. [Codex qualification](CODEX_QUALIFICATION.md) records native discovery and one synthetic request, but production discussions remain mock-only and live-provider support is unqualified. The current W6 development slice adds saved-version comparison and explicit document restore under [ADR 0005](ADR_0005_DOCUMENT_HISTORY.md); final W6 qualification remains pending. W5 does not close the B trial, C4–C6, or the broader V3 qualification gates.
