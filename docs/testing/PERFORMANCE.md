...
- **Results**: `windows-native` producer — poisoned-warm opt2 `26m57s`, cold opt2 `34m55s`, warm opt2 `8m23s`, cold opt0 `16m29s`, warm opt0 `9m48s`. Warm step timings at opt0: dependency restore `68s`, clippy `34s`, `cargo test` `4m43s` (workspace rebuild + execution), `desktop:spike` `1m17s`. Fully green end-to-end run: `14m04s` vs `~34m` on the poisoned-cache baseline. Cold builds gain the most from opt0 (dependency codegen dominates there); warm runs are equivalent within noise, and a `Cargo.lock` change still implies a cold run, so the cold-build saving is the operative one.
...
- **Reopening Condition**: Reopening requires hosted-runner timing data showing the `opt-level = 0` override increases time-to-green (e.g. slower native test execution outweighing codegen savings) or evidence the override leaks into release builds. One `native-consumer (main)` timing flake (`memory_views` row count read before the write landed) occurred once on a cold opt0 run and did not recur on the warm opt0 run; if it repeats preferentially under opt0, that is evidence to revisit dep-level optimization for the dev profile that builds the tested app.
- **Evidence**: workflow runs `35397231323` (cold opt2 populate), `35400445360` (warm opt2 + source edit), `35403190753` (cold opt0 populate), `35404930115` (warm opt0 + source edit, fully green) on `FZWINGEL/WebnovelStudio_V3`; `cargo-timings-windows-*` artifacts on each run.
