...
### 3. Dependency Cache and Compile Budget

The producer jobs cache Cargo dependencies with `Swatinem/rust-cache`. The computed key covers the runner OS/architecture, the Rust toolchain, `CARGO_*`/`RUST_*` environment variables, and a hash of `.cargo/config.toml`, every workspace `Cargo.toml`, `Cargo.lock`, and `rust-toolchain.toml`. Ordinary source edits (`.rs`, `.ts`) never change the key.

- **Entries are immutable.** An entry written by a run that stopped before compiling every required variant still exact-matches on later runs and can never be republished, so `cache-on-failure` stays `false` and only a fully green producer may publish. To force a fresh entry after changing CI cargo configuration that is not already hashed, bump the `-gN` generation suffix in `shared-key` — that is the documented cache-reset procedure.
- **CI-only dependency optimization.** The manifest keeps `opt-level = 2` for dependencies so local debug runs stay fast. Hosted CI appends an `opt-level = 1` package override to `.cargo/config.toml` (marked `skip-worktree` so the native-artifact checkout-integrity gate still sees a clean tree) before the cache step — `1` rather than `0` because the debug build under test showed elevated consumer-side timing races with fully unoptimized dependencies. Ordering matters: the file is hashed at restore time, so the override produces a distinct cache entry instead of colliding with an `opt-level = 2` one, and the same checkout config also reaches the cargo build spawned by the Tauri CLI.
- **Two dependency variant sets are expected.** `cargo test --workspace` builds the default feature set while `desktop:spike` enables Tauri's `custom-protocol` feature, so a complete cache legitimately contains both compiled variants; the second build reuses shared dependencies and only recompiles the feature-divergent subtree. Unifying the feature sets or stripping dev-dependencies to deduplicate is not an option — it would change what is being verified.
