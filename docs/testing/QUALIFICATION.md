...
- **CI-only dependency optimization.** The manifest keeps `opt-level = 2` for dependencies so local debug runs stay fast. Hosted CI appends an `opt-level = 0` package override to `.cargo/config.toml` (marked `skip-worktree` so the native-artifact checkout-integrity gate still sees a clean tree) before the cache step. Ordering matters: the file is hashed at restore time, so the override produces a distinct cache entry instead of colliding with an `opt-level = 2` one, and the same checkout config also reaches the cargo build spawned by the Tauri CLI.
