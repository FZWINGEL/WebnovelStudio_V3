...
**Status:** adopted design. C0–C2 and parts of C3 are implemented: exact evidence/retrieval, frozen packets, adopted guidance, recent complete exchanges, linked retry guidance, saved discussion sources, and optional approved writing briefs. C4-A adds explicit single-chapter navigation memory with retained evidence and source inspection; C4-B supplies eligible current views to working author-room discussions when full prose does not fit, using immutable generated-view references, separate coverage, and exact source inspection. Selected-passage and structured Apply/Reject, history restore, draft export, bounded Codex assistance, author-only chapter review, schema-19 continuation, schema-21 reviewed evidence, schema-23 promise history, and C5-A evidence history are integrated into the native development app at documented development boundaries. The C6 bounded lookup implementation is an opt-in `story-lookup.v1` route for Working, AuthorRoom, and Discuss, with schema-24 invocation/read persistence and focused protocol, packet, frontend, and local native evidence; its accepted wrapper checkpoint passes, while native accepted-build, hosted, and live-provider qualification remain open. Review fixes cover crash/reopen outcomes, exact child-run provenance, terminal/result/assistant consistency, historical unavailable gaps, reproducible search metadata, and truthful delivery status. The route is documented in [ADR 0022](ADR_0022_BOUNDED_STORY_LOOKUPS.md). Promise history is an implemented development slice; current native, bounded live, and CI evidence is maintained in [implementation status](IMPLEMENTATION_STATUS.md). Broader live coverage, author-trial, and full provider/native/release qualification remain open. The F2-B core freezes exact earlier reviewed versions plus a current working target, with immutable historical reader positions. C3 relevance selection, higher-level C4 digests, broader C5 state views, restricted-writing lookup, typed accepted story records, and narrative evaluation remain open. Exact current test and hosted evidence is maintained in [implementation status](IMPLEMENTATION_STATUS.md).
...
Current implementation and qualification evidence is maintained in [implementation status](IMPLEMENTATION_STATUS.md). The supplied research and proposal remain design rationale; they are not substituted for executed tests or provider qualification.
