...
**Status:** adopted design; C0–C2, C3 author guidance, recent complete exchanges, linked retry guidance, and saved discussion sources are implemented. Selected-passage Apply/Reject, history restore, and draft export are implemented. Safe briefs, richer relevance selection, C4–C6, production providers, and benchmark execution remain open. Current local and hosted native evidence is recorded in [implementation status](IMPLEMENTATION_STATUS.md).
