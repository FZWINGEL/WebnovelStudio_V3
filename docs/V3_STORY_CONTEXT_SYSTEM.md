...
**Status:** adopted design; C0–C2, C3 author guidance, recent complete exchanges, linked retry guidance, saved discussion sources, and the optional author-approved writing brief are implemented locally. Selected-passage Apply/Reject, history restore, and draft export are implemented. C3 remains partial because richer relevance selection is open. Core checks and the local native diagnostic pass; hosted qualification remains pending; the current embedded Tauri build has a 25-check local diagnostic on WebView2 `152.0.4191.62`, with only clipboard omitted from the strict 26-check flow. C4–C6, production providers, and benchmark execution remain open. Current evidence is recorded in [implementation status](IMPLEMENTATION_STATUS.md).
