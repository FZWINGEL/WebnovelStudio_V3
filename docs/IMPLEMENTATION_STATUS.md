...
- The implementation is pushed as `73db1bc03e9a99eef1e5cdd31d8dbcb9be0dac53` on `codex/v3-persistence`. Its [CI run 33973213684](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33973213684) passed the Windows native, Windows, and Ubuntu contract jobs.
- The follow-up [CI run 33973390433](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33973390433) for `6456d33` passed the contract jobs but failed the native duplicate-edit check. The native smoke helper now waits for the actual ProseMirror transaction after contenteditable fill; a new run for that fix is pending and has not been pushed at this record's update.
...
| W3 library/transfer | Active work | Registry, transfer, schema-3 migration with durable pre-upgrade backup and schema-1/2 recovery, historical-snapshot authority fencing, and persistent Library/Workspace flows are underway/partly wired | Complete library/recovery/A-trial contract and evidence |
| GitHub/CI | Implementation pushed; native follow-up pending | Private repo, `main` default, `73db1bc` run 33973213684 passed Windows native, Windows, and Ubuntu contracts; follow-up run 33973390433 exposed a native duplicate-edit failure | Push and record the rerun after the ProseMirror transaction wait fix |

## Current local evidence

The root wrapper passed rustfmt, workspace Clippy with `-D warnings`, 90 active Rust tests plus one ignored child test (the child is invoked by its parent), the TypeScript/Vite build, and 79 frontend tests. The focused `createIntent` suite also passed 7/7, and the full desktop Vitest rerun passed 80/80. Review regressions cover scene-break caret saving, empty-project reconciliation, stable create-retry IDs, mounted-editor lease adoption, generic lost acknowledgments, ignoring an unavailable last document, preserving the current writer lease when destination reads fail, and view/metadata acknowledgment fences. The prior rebuilt real Tauri/WebView2 smoke baseline passed 14/14 checks on WebView2 `152.0.4191.62`, covering two-project save/switch/reload and copy isolation, process kill/restart, project/document rename, exact caret/last-document restore, and archive/unarchive. That baseline is historical: the later native follow-up failed the duplicate-edit check, and the corrected helper has not been rerun locally while the debug executable remains open. The ignored `.local/native-results/report.json` is machine-local evidence. A prior native Windows dialog check saved a 55,151-byte synthetic backup through the Backup action. Native export/recovery dialogs, normal-close behavior, and the broader A, N, and W3 qualification gates remain open.
...
Integrate the C0–C3 context foundation before or alongside this package: frozen snapshots and exact eligible sources, source epoch, deterministic exact retrieval with dirty-index fallback, mandatory-budget refusal, scoped author guidance, and actual-packet receipts/inspector. C0 and C1 are implemented in the core; C2 is in progress and C3 remains planned. These context packages do not replace the base save, Apply, lifecycle, or authority contracts.
