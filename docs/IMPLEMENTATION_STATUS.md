...
The native development app supports persistent projects, free-order English writing, document discussion, exact context inspection, adopted guidance, saved discussion sources, optional approved writing briefs, selected-passage Apply/Reject, saved versions, exact Markdown/TXT export previews, bounded Windows Codex assistance, and independent V2 schema-8 import. Author-only chapter review stages exact saved prose and its earlier reviewed basis for explicit acceptance. The F2-B core slice freezes that exact reviewed prefix plus the current working target over IPC and retains immutable reader-position pins. Projects use schema 15; app-local model preferences remain library schema 2. C0–C2, parts of C3, and the F2 review/context core are implemented. Continuation UI/live dispatch, ready export, typed reviewed-story records, C4–C6, broader Apply, narrative evaluation, and full native/release gates remain open.
...
| W3 library/transfer | Implemented development surface | Registry, rename/archive/duplicate, isolated recovered projects, migration backups, persistent library/workspace, and schema-15 backup validation | Complete the A author trial and broader recovery/native-dialog evidence |
...
| W7 exports/package | Development slice and narrow installed lifecycle passed | Frozen Markdown/TXT preview, exact native Save, immutable export records, stable release data, strict native28 baseline, installed lifecycle CI33994616334 | Offline/no-runtime installation, true upgrade, physical/assistive native trials, full N gates |
...
| F2 author review | Author-only prose and reviewed-context core implemented | Exact stages/revisions/earlier prefix, explicit selected bundles, immutable reader-position pins, current validity, saved-stage resumption, independent recovered history, core/IPC freeze of reviewed prefix plus current target | Continuation UI/live dispatch, ready export, typed records/summaries/exceptions, and full F2 qualification |
| GitHub/CI | F2-A checkpoint passed; F2-B not pushed | `c3f0f83` passed CI 33998398286 with all 30 strict native checks; this evidence predates the F2-B core changes | Push and validate the reviewed-context checkpoint when the core/native gate is ready |
...
- [ ] Extend these bundles with typed accepted rules/records, optional accepted summaries, issue decisions/exceptions, and known dependency evidence. Current conservative prefix/suffix handling does not establish complete continuity.
