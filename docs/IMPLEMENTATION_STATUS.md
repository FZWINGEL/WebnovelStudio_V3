...
The native development app supports persistent projects, free-order English writing, document discussion, exact context inspection, adopted guidance, saved discussion sources, optional approved writing briefs, selected-passage Apply/Reject, saved versions, exact Markdown/TXT export previews, bounded Windows Codex assistance, and independent V2 schema-8 import. Author-only chapter review stages exact saved prose and its earlier reviewed basis for explicit acceptance. The F2-B core slice freezes that exact reviewed prefix plus the current working target over IPC and retains immutable reader-position pins. The C4-A Story Memory slice provides explicit, source-linked navigation digests for one full current chapter revision. Projects use schema 16; app-local model preferences remain library schema 2. C0–C2, parts of C3, the F2 review/context core, and the C4-A development slice are implemented. Continuation UI/live dispatch, ready export, typed reviewed-story records, broader C4 derived-view packet integration, C5/C6, broader Apply, narrative evaluation, and full native/release gates remain open.
...
| C4-A chapter memory | Implemented development slice | Explicit Refresh for one full current chapter; schema 16 job/result/view records; strict evidence validation; recovery/cleanup regressions; native diagnostic and one live Codex memory request | Strict checkpoint CI, derived-view packet integration, broader provider/native qualification, and narrative quality remain open |
| GitHub/CI | F2-B checkpoint passed | `6618f6e` passed CI 33999551379 with all 31 strict native checks | C4-A is a newer local development slice; final qualification is pending |
