...
The native development app supports persistent projects, free-order English writing, mock discussions, exact context inspection, adopted guidance, saved discussion sources, optional approved writing briefs, selected-passage Apply/Reject, saved-version restore, and frozen Markdown/TXT export previews. C0–C2 and parts of C3 are implemented. [CI run 33990073110](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33990073110) for `16bf8bc` passed Windows/Ubuntu contracts and the strict 25-check native flow on WebView2 `151.0.4129.101`. The newer writing-brief flow passed 25 local diagnostic checks on WebView2 `152.0.4191.62`, with only clipboard omitted; its core checks and local diagnostic pass, while hosted qualification remains pending. Production discussions remain mock-only. Installed-release, full V3, and narrative-quality gates remain open.

The Windows child-process foundation and pure Codex JSONL parser are implemented without a production dispatch connection. Current work completes C3 writing-brief verification and qualifies the opt-in installed-package lifecycle. Release builds hide the session-only editor trial.
...
| GitHub/CI | C3 saved-source strict native and both contract jobs passed; later restart harness race corrected locally | `16bf8bc`, CI33990073110, WebView2 `151.0.4129.101` | Qualify writing briefs, corrected restart wait, and installed release |

## Current local evidence

The final writing-brief wrapper passes rustfmt, workspace Clippy with `-D warnings`, **243 active Rust tests** (242 core and one desktop; one ignored subprocess target), TypeScript/Vite, and **171 frontend tests**. The embedded Tauri build succeeds with a 627.57 KB bundle and the existing size warning. At `2026-09-05T20:45:34.100Z`, the 25-check native diagnostic passed on WebView2 `152.0.4191.62`, omitting only clipboard. It verified explicit approval, retained draft approval after reopening, exact delivered directions without original private planning, context inspection, and unchanged prose before Apply. The native approval screenshot was independently reviewed and accepted; physical DPI, screen-reader use, and author trials remain unqualified. A final rebuilt diagnostic includes the corrected Library wait before process termination. All compatibility regressions pass; hosted native26 qualification remains pending.
...
The unsigned Windows x64 NSIS package builds with the bundled offline WebView2 installer and stable release data path. Hosted package run 33990404236 built and installed `1e6e556`, opened the release Library, created synthetic prose, and read it back, then failed before returning to Library. Its owned-window capture showed an uncertain save. Source inspection identified `validate_snapshot` incorrectly guarded by `debug_assertions`, although production autosave calls it. The release command registration is being corrected and an actual release executable is being tested. Normal close/uninstall/reinstall were not reached in that failed run and cleanup was forced. See [Windows package qualification](WINDOWS_PACKAGE_QUALIFICATION.md).
