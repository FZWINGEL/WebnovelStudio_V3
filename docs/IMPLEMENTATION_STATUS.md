...
The native development app supports persistent projects, free-order English writing, mock discussions, exact context inspection, adopted guidance, explicit selected-passage Apply/Reject, and saved-version restore. C0–C2 context foundations and parts of C3 are implemented. W5 has passing Windows, Ubuntu, and native CI. W6 is pushed; its native CI exposed a background caret-save focus race, addressed in the current W7 work alongside exact Markdown/TXT export previews, schema-9 export records, and Windows installer configuration. The application still uses the deterministic mock; separate Codex qualification experiments do not enable a production adapter. See the full W0–W8/F1–F5/C0–C6 ledger below for remaining work. These development results do not qualify an installed release or narrative quality.
...
| GitHub/CI | W6 pushed; native regression under repair | `1a6beb7`, CI33982596342: both contract jobs passed; Redo timed out in native flow | Publish and verify the W7 repair checkpoint |
...
The W7 diagnostic reached the delayed-caret acknowledgment, history/restore, exact export preview, and real native Cancel flow. Local Save automation could set and read back the exact synthetic destination, but the dialog did not close after its verified button action. A pointer ownership diagnostic refused to click an unrelated window. The tracked helper uses only owned UI Automation controls or verified native control messages, with bounded completion checks; it has no global pointer/keyboard fallback. This leaves native Save qualification open pending the strict hosted Windows run.
