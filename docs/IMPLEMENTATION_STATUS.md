...
- W4 checkpoint `e92eef8a212312972609057c61d6667db8d27c5f` is pushed and adds persistent discussion threads, selected scope grants, drafts, deterministic mock jobs, durable output events, Stop/failure/interruption recovery, and packet inspection. The last committed W5 predecessor `9161fc84174ed8922f3e6363c98688afe1acda03` (`9161fc8`) has [CI run 33979310784](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33979310784) green across Windows, Ubuntu, and Windows native (native19). This W5 checkpoint has no integrated full/native qualification result; its diagnostic native subset is recorded below. Earlier checkpoint and failure records remain below as historical gates.
...
| W5 proposal review and Apply | Active W5 checkpoint | Schema-8 proposal records, explicit Discuss/ProposeEdits intent, restricted chapter-passage packets, strict 1–3 candidate terminal/raw retention, immutable prepared versions and decisions, exact CAS validation, atomic Apply/Reject receipts, stale/replay fences, and editor pending-Apply handoff are implemented; focused tests and a diagnostic native subset pass, while the strict full native flow remains open | Integrated full/native qualification; broader whole-chapter/block/batch/manual-rebind/restore Apply contracts remain open |
| GitHub/CI | Context, guidance, recent discussion, and retry checkpoints pushed | Last committed checkpoint `9161fc8` is covered by CI run 33979310784 across Windows, Ubuntu, and Windows native (native19). This W5 checkpoint has no integrated qualification result | Publish/qualify W5 and record its integrated checks |

## Current local evidence

The latest full wrapper check passed rustfmt, workspace Clippy with `-D warnings`, 168 active Rust tests (167 core plus one desktop test), the TypeScript/Vite build, and 122 frontend tests. The local C2/C3 evidence includes 20 compiler, 28 discussion, 4 guidance, 9 receipt, and 6 migration tests. A fresh local C3 real Tauri/WebView2 smoke checkpoint passed 19/19 checks on WebView2 `152.0.4191.62`, including guidance, exact prior-exchange context, Stop/retry/reload retention, and receipt inspection; this is local development evidence, not release qualification. Review regressions cover scene-break caret saving, empty-project reconciliation, stable create-retry IDs, mounted-editor lease adoption, generic lost acknowledgments, late guidance responses, and view/metadata acknowledgment fences. The W5 focused evidence includes 10 core proposal tests plus one real-process-kill Apply test and 9 frontend Apply tests plus two ProseMirror history tests; these focused checks pass. After the reload wait was fixed to await the `Your stories` heading, a diagnostic Tauri/WebView2 W5 subset passed 20/20 on WebView2 `152.0.4191.62`: it covered Preview edited “Her sister”, preview retention across navigation/reload, Apply against the same editor and protected ending, two other proposals remaining stale/pending, Reject without change, and Ctrl+Z/Ctrl+Shift+Z with body and decisions retained after reload. `proposal-preview.png` and `proposal-applied.png` were visually inspected and readable. The strict full native21 flow remains failed at the existing W0 clipboard step in both current and old binaries: copy serializes correctly, but paste receives an empty `DataTransfer`; a 100 ms yield did not fix it, and no skip was added. The observed clipboard cause remains unresolved; a diagnostic `Get-Clipboard -Raw` query also failed, and no competing clipboard holder was found. The ignored `.local/native-other-results` output is the explicit subset and omits clipboard coverage. The Vite build reports a 599.44KB bundle warning. Native export/recovery dialogs, normal-close behavior, packaged installation, accessibility, live providers, and the broader A, N, and W3 qualification gates remain open.
...
**Gate:** M/P; B trial remains blocked until W6. **Status:** active W5 checkpoint; integrated qualification pending.
...
The parent editor session flushes and preflights behind a pending-Apply barrier, commits the durable operation before dispatching the exact existing-editor transaction with `closeHistory`, retains newer edits on conflict, and reconciles uncertain acknowledgments without autosave. Native IPC mock commands are wired. Focused W5 tests and the diagnostic native subset pass locally; the strict full native21 flow still stops at the existing W0 clipboard Ctrl+V smoke step before reaching W5. Whole-chapter/block/batch/manual-rebind/restore Apply contracts remain open.
