...
The next development checkpoint adds a Windows child-process foundation and pure Codex JSONL parser, with no production dispatch connection. Installed builds now hide the session-only editor trial, and an opt-in hosted package lifecycle is being qualified.
...
The latest full wrapper check passed rustfmt, workspace Clippy with `-D warnings`, **218 active Rust tests** (217 core and one desktop; one ignored subprocess entry), TypeScript/Vite, and **154 frontend tests** on Windows 11 Pro `10.0.26200`. A separate desktop release compile check passed, including the development-only trial IPC gate. The bundle is 615.46 KB and retains Vite's size warning. The portable parser is exported on every target; the Windows process module remains platform-gated.
...
Hosted run 33987335890 exposed an intermittent ExportDialog preview test wait on Ubuntu (153/154 frontend tests passed). The test now yields asynchronous hashing and React commits under `act` with a bounded two-second wait. Its ten focused tests pass; five repeated checks passed. This is a test synchronization correction; no production export behavior changed. Current package source `4a59a19` has passing Windows/Ubuntu contract jobs in [run 33987542771](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33987542771); native execution is still pending. The separate [package lifecycle run 33987543086](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33987543086) is building the installer. Neither pending run is recorded as a pass.
