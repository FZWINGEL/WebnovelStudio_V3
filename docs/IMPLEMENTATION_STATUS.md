...
| C4-A chapter memory | Implemented development slice; CI-covered | Explicit Refresh for one full current chapter; schema 16 job/result/view records; strict evidence validation; recovery/cleanup regressions; native CI and one live Codex memory request | Derived-view packet integration, broader provider/native qualification, and narrative quality remain open |
| GitHub/CI | C4-A checkpoint passed | `7a27135` passed CI 34003001409 with all 34 strict native checks | Requalify subsequent source changes; no full V3/release completion claim |
