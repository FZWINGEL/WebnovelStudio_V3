...
The native development app supports persistent projects, free-order English writing, mock discussions, exact context inspection, adopted guidance, saved discussion sources, optional approved writing briefs, selected-passage Apply/Reject, saved-version restore, frozen Markdown/TXT export previews, and a persistent model selector/Settings surface. C0–C2 and parts of C3 are implemented. [CI run 33990073110](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33990073110) for `16bf8bc` passed Windows/Ubuntu contracts and the strict 25-check native flow on WebView2 `151.0.4129.101`. The writing-brief checkpoint `508aee1` passed [CI run 33991463598](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33991463598), including Windows/Ubuntu contracts and all 26 strict native checks on WebView2 `151.0.4129.101`. The latest native diagnostic passed 27 of 28 checks on WebView2 `152.0.4191.62` at `2026-09-05T21:52:10.597Z`, omitting only known local clipboard coverage. The current tracked harness contains 28 checks; this diagnostic covered the model-selection flow. Production discussions remain mock-only. Broader installed-release, full V3, and narrative-quality gates remain open.

The Windows child-process foundation, incremental observer, and pure Claude/Codex JSONL parsers are implemented without a production dispatch connection. The current local slice adds bounded provider streaming and cleanup-aware Stop behavior, while the opt-in installed-package lifecycle remains under qualification. Release builds hide the session-only editor trial. Model preferences are persisted in library schema 2 while author projects remain schema 11; only the local Mock model is ready, and unavailable Codex choices block generation without CLI probes, credential reads, or new live requests.
...
| W7 exports/package | Active development slice | Exact frozen Markdown/TXT preview, explicit native Save, schema-9 immutable export records, stable release data/NSIS configuration, strict hosted native24 evidence for exact export and focus, and the narrow installed lifecycle in package run 33993370498 | Offline installation, upgrades, full packaged WebView qualification, and broader N gates |
| W8 provider foundation | Local checks pass; no live adapter enabled | Job containment, bounded stdin/output, incremental observer (20 integration tests and 3 module tests), typed partial failures, 12 pure Claude parser tests, 13 pure Codex parser tests, and model preference persistence with only Mock dispatchable | Provider configuration, streaming integration, live token budgeting, complete isolation evidence, and W8/E3 acceptance |
| GitHub/CI | Contract jobs pass; current native UI run remains open | `cd7fb77`, CI33993367813 passed Windows/Ubuntu contracts but failed the descendant cleanup regression before UI; package run 33993370498 passed the narrow installed lifecycle | Fresh native rerun after the selector fix, broader package qualification, and later W8 changes |

## Current local evidence

The final local wrapper checkpoint passed rustfmt, workspace Clippy with `-D warnings`, **297 active Rust tests** (286 core and 11 desktop; one ignored subprocess target), **185 frontend tests**, and TypeScript/Vite. The embedded Tauri build also passed. The bundle is 638.70 KB with 26.71 KB CSS and the existing Vite size warning. At `2026-09-05T21:52:10.597Z`, the native diagnostic passed 27 of 28 checks on WebView2 `152.0.4191.62`, omitting only known local clipboard coverage. The current tracked harness contains 28 checks; this diagnostic covered model choice, favorites, High/Fast trait persistence across reload, exact Rust IPC state, and blocked Codex dispatch without substitution. Subsequent importer/profile hardening is covered by the final local wrapper; those components are not exposed in the native UI or enabled for dispatch. Physical DPI, screen-reader use, and author trials remain unqualified. The earlier 277/178 and 296/185 wrapper counts and 26/27 diagnostic remain historical checkpoint evidence.

The [V2 preview foundation](V2_IMPORT_PREVIEW.md) is implemented as a Windows-only read-only core API with eight synthetic tests. It validates schema 8 and record ownership, uses an owned stable copy, preserves missing versus empty prose and inert history, and refuses non-Windows source access. Staged installation, legacy evidence persistence in V3, and an author-facing Import action remain open. The pure exact-version Codex launch-profile constructor has four tests and documented requested tool/configuration controls; it has not been run against the executable and does not enable live assistance.

The model preference slice is locally covered by seven Rust core tests, five header-picker/Settings tests, two FeedbackPanel binding tests, and one desktop model-binding test. Library schema 2 stores non-secret preferences while author projects remain schema 11. Only the Mock model is ready; unavailable Codex choices block generation and do not trigger CLI probes, credential reads, or new live requests. The current native evidence above verifies model choice, favorites, High/Fast trait persistence across reload, exact Rust IPC state, and no silent Codex substitution.
