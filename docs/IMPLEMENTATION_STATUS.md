...
The native development app supports persistent projects, free-order English writing, mock discussions, exact context inspection, adopted guidance, and explicit selected-passage proposals with durable Apply/Reject. C0–C2 context foundations and parts of C3 are implemented. W5 is pushed and has passing Windows, Ubuntu, and native CI. This W6 checkpoint adds saved-version comparison and explicit restore through the shared lifecycle/reconciliation path. The application still uses the deterministic mock; one separate synthetic Codex request has succeeded, with production-provider qualification still open. See the full W0–W8/F1–F5/C0–C6 ledger below for remaining work. These development results do not qualify an installed release or narrative quality.
...
| GitHub/CI | W5 pushed and green | `a2a0163`, CI33981203728: Ubuntu, Windows, strict native21 | Publish W6 and verify its exact CI result |

## Current local evidence

The latest full wrapper check passed rustfmt, workspace Clippy with `-D warnings`, **175 active Rust tests** (174 core and one desktop; one ignored subprocess target), TypeScript/Vite, and **138 frontend tests**. The bundle is 608.30 KB and retains Vite's size warning.
...
One isolated synthetic Codex CLI `0.153.3` request completed in about 12 seconds with no observed tool events. It requested Luna/Max/Fast (`priority`); the response did not independently echo effective model or traits. The ineffective unified-exec override and outstanding containment/Stop/failure gates are recorded in [Codex qualification](CODEX_QUALIFICATION.md). No live adapter is enabled in the application.
