...
- W4 checkpoint `e92eef8a212312972609057c61d6667db8d27c5f` is pushed and adds persistent discussion threads, selected scope grants, drafts, deterministic mock jobs, durable output events, Stop/failure/interruption recovery, and packet inspection. [CI run 33976322600](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33976322600) completed successfully across Windows native, Windows, and Ubuntu contract jobs. C3 guidance and packet-integrity checkpoint `97e9511e264f1ecc98575dfc691bb68960f8c6fe` is pushed. Its [CI run 33977815078](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33977815078) passed all three jobs. Recent discussion checkpoint `ec87b769463d9a1c19af5c13518842fd2c44a451` is pushed; [run 33978480373](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33978480373) passed both contract jobs but its native job failed on an immediate focus assertion in the sample editor. The label already existed while the focus handoff was scheduled for the next animation frame. The current smoke waits for the actual focused element and passes 19/19 locally. Retry checkpoint `7cad1cd2f69e5a4c62a01b6f2ce7d520a779d032` is pushed; [run 33978993068](https://github.com/FZWINGEL/WebnovelStudio_V3/actions/runs/33978993068) has green contract jobs with its native result pending at the last check.
...
| W3 library/transfer | Active work | Registry, transfer, schema-7 migration with durable pre-upgrade backup and schema-1/2/3 recovery, isolated recovered operation namespaces, historical-snapshot authority fencing, and persistent Library/Workspace flows are underway/partly wired | Complete library/recovery/A-trial contract and evidence |
...
**Gate:** M/P; B trial remains blocked until W6. **Status:** active; pure fragment validation implemented locally.

Rust now validates the exact proposed text and inherited formatting in addition to every protected token outside the selection. The first fragment grammar is single-line text over a passage, including same-style paragraph merges. Uniform selected marks are inherited; mixed marks yield plain replacement text. Scene breaks, mismatched block styles, structural grants, and oversized/newline fragments require later grammar work. Four tests cover agreement with existing ProseMirror golden results, altered candidate text/marks, protected neighbours, mixed formatting, Unicode, deletion, and bounds. Durable proposal/decision records, a restricted prose-request path, editor handoff, and the user-facing Apply flow remain to be implemented.
