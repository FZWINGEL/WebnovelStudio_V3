...
The native development app supports persistent projects, free-order English writing, document discussion, exact context inspection, adopted guidance, saved discussion sources, optional approved writing briefs, selected-passage Apply/Reject, saved versions, and exact Markdown/TXT export previews. The current integration adds a bounded Windows Codex path and independent V2 schema-8 import. Projects use schema 13; app-local model preferences remain library schema 2. C0–C2 and parts of C3 are implemented. Reviewed-story authority, C4–C6, broader Apply, narrative evaluation, and full native/release gates remain open.
...
| W3 library/transfer | Implemented development surface | Registry, rename/archive/duplicate, isolated recovered projects, migration backups, persistent library/workspace, and schema-13 backup validation | Complete the A author trial and broader recovery/native-dialog evidence |
...
| F1 V2 import | Implemented schema-8 development slice | Explicit reviewed body choices, independent staged installation, inert history, Library Check import, source-free receipt recovery, and full pre-move validation | Native import journey and representative author-approved acceptance |
| GitHub/CI | Baseline green; new integration pending | be1d93c passed Windows/Ubuntu contracts, strict native28, and narrow installed lifecycle | Commit and validate the live-provider/import checkpoint |

## Current local evidence

The final integrated local wrapper passed rustfmt, workspace Clippy with `-D warnings`, **325 active Rust tests** (310 core and 15 desktop; one ignored subprocess fixture), TypeScript/Vite, and **194 frontend tests**. This includes frozen response-format and live receipt/backup guards, plus durable Library import recovery and read-only pre-install validation. The bundle is 649.21 KB JavaScript and 28.15 KB CSS, with the existing Vite size warning. The rebuilt native development executable passed compilation.
