...
- This W4 checkpoint adds persistent discussion threads, selected scope grants, drafts, deterministic mock jobs, durable output events, Stop/failure/interruption recovery, and packet inspection. Its local smoke and test evidence is recorded below; remote W4 CI is not yet claimed.
...
| W3 library/transfer | Active work | Registry, transfer, schema-5 migration with durable pre-upgrade backup and schema-1/2/3 recovery, isolated recovered operation namespaces, historical-snapshot authority fencing, and persistent Library/Workspace flows are underway/partly wired | Complete library/recovery/A-trial contract and evidence |
| W4 persistent discussions | Active local slice | Rust-owned threads/messages/runs/drafts, selected scope grants, frozen packet receipts, deterministic mock output, durable sequence/terminal events, Stop/retry/reload recovery, and native FeedbackPanel/ContextInspector integration | Finish malformed-output/cancellation/error/UI coverage, guidance and permitted history compilation, then durable Apply and broader W4 evidence |
| C2 context packet compiler | Implemented and pushed; native development smoke covered | Rust pure deterministic compiler, all-source validation, exact target/instruction/scope/mandatory-pin preservation, full eligible text when it fits, stable whole-block prefix packing with explicit omissions otherwise, durable exact packet/options/hash receipts, and native IPC; 13 compiler, 7 receipt, and 6 context migration tests pass | Qualify provider tokenization separately; C3 guidance/history and W4 completion remain open |
| GitHub/CI | C0–C2 pushed | Private repo, `main` default, pushed `cd1d8a6` CI run 33975322590 completed successfully across Windows native, Windows, and Ubuntu contracts; W4 local changes are not yet published | Publish the W4 checkpoint and record its CI result |

## Current local evidence

The root wrapper passed rustfmt, workspace Clippy with `-D warnings`, 122 active Rust tests plus one ignored child test (the child is invoked by its parent), the TypeScript/Vite build, and 86 frontend tests. The W4 additions include 11 discussion lifecycle tests; the C2 additions include 13 compiler tests, 7 durable-receipt tests, and 6 context migration tests. Review regressions cover scene-break caret saving, empty-project reconciliation, stable create-retry IDs, mounted-editor lease adoption, generic lost acknowledgments, ignoring an unavailable last document, preserving the current writer lease when destination reads fail, and view/metadata acknowledgment fences. A fresh isolated real Tauri/WebView2 smoke flow passed 16/16 checks on WebView2 `152.0.4191.62`, including persistent discussion, selected feedback, context receipt inspection, draft/history retention across project switch and renderer reload, and unchanged editor content. The Vite build still reports a 569.44KB bundle warning. The ignored `.local/native-results/report.json` is machine-local evidence. A prior native Windows dialog check saved a 55,151-byte synthetic backup through the Backup action. Native export/recovery dialogs, normal-close behavior, packaged installation, accessibility, live providers, and the broader A, N, and W3 qualification gates remain open.

## Full completion checklist

C2 review added explicit scope requirements for prose-producing purposes, mandatory evidence-dependency closure, non-sensitive policy exclusion counts in the delivered envelope, and a 10,000-block regression for bounded packing memory. The context inspector is mounted in the persistent discussion surface, and source pins are captured. Preparing or reading a packet does not authorize dispatch: W4 atomically claims a run against current source/policy epochs before invoking the deterministic mock. Keep as guidance and permitted recent conversation-history compilation are not yet implemented. No durable Apply or live provider is connected.
...
| C3 | Before/alongside W4; scoped author guidance and the context inspector | Partial: inspector/source pins integrated | Used/Available/Not included/Needs refresh views show packet metadata, omissions, freshness, and exact source reads; W4 captures pins for the next request. Guidance scope/versioning, persistent chapter/project pins, and permitted conversation-history compilation remain open |
