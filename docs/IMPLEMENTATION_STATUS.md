...
The native development app supports persistent projects, free-order English writing, document discussion, exact context inspection, adopted guidance, saved discussion sources, optional approved writing briefs, selected-passage Apply/Reject, saved versions, exact Markdown/TXT export previews, bounded Windows Codex assistance, and independent V2 schema-8 import. Author-only chapter review now stages exact saved prose and its earlier reviewed basis for explicit acceptance. Projects use schema 14; app-local model preferences remain library schema 2. C0–C2 and parts of C3/F2 are implemented. Typed reviewed-story records and context resolution, C4–C6, broader Apply, narrative evaluation, and full native/release gates remain open.
...
| W3 library/transfer | Implemented development surface | Registry, rename/archive/duplicate, isolated recovered projects, migration backups, persistent library/workspace, and schema-14 backup validation | Complete the A author trial and broader recovery/native-dialog evidence |
...
| F2 author review | Author-only prose slice implemented | Exact stages/revisions/earlier prefix, explicit selected bundles, current validity, saved-stage resumption, independent recovered history, and native diagnostic review journey | Strict CI confirmation, typed records/summaries/exceptions, reviewed context resolver, and ready export |
| GitHub/CI | New author-review checkpoint pending | 5689597 passed strict native28 and installed lifecycle; its Ubuntu Clippy failure is corrected locally | Push and validate author review plus the cross-platform correction |
