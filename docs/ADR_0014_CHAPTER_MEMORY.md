...
Installing a generated aid does not advance the story-source epoch. This avoids making ordinary analysis invalidate the source it just read. Editing story material still advances the epoch and makes previously frozen requests and proposals stale. C4-C resolves this closed single-chapter recipe against its exact saved chapter instead: unrelated writing does not stale the view, while a changed source chapter does. The original generation epoch remains provenance. This exception does not apply to contextual or cross-chapter interpretations. Source change and disclosure revocation are different: stale source evidence may remain inspectable, whereas revoked material and diagnostics that might quote it are redacted from request-facing reads.
...
The original C4-A cut exposed generated memory for author inspection. [ADR 0015](ADR_0015_NAVIGATION_CONTEXT.md) extends it with separate frozen view identities and bounded automatic use in working author-room discussions. That integration has its own qualification record. A generated view must never masquerade as a manuscript `SourceRef` or alter the bytes denoted by a revision hash.

The C4-B increment freezes the chosen view identity, content fingerprint, complete source dependencies, disclosure policy, and coverage label. It preserves exact task text, distinguish digest coverage from verbatim prose, omit stale or ineligible views, and retain the exact view for already frozen requests. Arc/story summaries and incremental rebuilds follow only after this contract works for chapter views. C5 temporal relationships and C6 bounded read-only model lookup remain separate work.
