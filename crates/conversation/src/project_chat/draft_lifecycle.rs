...
fn validate_response_scope(
    ...
    item_kind: &str,
    disposition: &str,
    scope: &crate::project_chat::ChatDispositionScope,
    unknown_to: Option<crate::project_chat::ChatUnknownTo>,
) -> CoreResult<()> {
    scope.validate_shape()?;
    match scope.kind {
        crate::project_chat::ChatDispositionScopeKind::Project => {}
        crate::project_chat::ChatDispositionScopeKind::Task => {
        ...
        crate::project_chat::ChatDispositionScopeKind::Chapter
        | crate::project_chat::ChatDispositionScopeKind::Document => {
            ...
            if scope.kind == crate::project_chat::ChatDispositionScopeKind::Chapter
...
fn draft_save_payload_hash(request: &SaveAssistantDraft) -> CoreResult<String> {
    ...
    Ok(sha256_hex(
        serde_json::to_string(&wns_kernel::canonicalize_value(value))?.as_bytes(),
    ))
}
...
host: &mut impl ProjectChatHost,
    ...
    if request.project_id != host.info().project_id
        || request.operation_namespace != host.info().operation_namespace
    {
        return Err(CoreError::new(
            "WrongProjectSession",
    ...
    host.recover_connection()?;
    let access = host.attach(request.session)?;
    let tx = host.db()?;
    ...
    for id in request.pending_operation_ids {
        let row: Option<(String, String, String)> = tx
            .query_row(
                ...
                params![host.info().operation_namespace, id, request.document_id],
