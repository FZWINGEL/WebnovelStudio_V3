...
use wns_storage::{CreationOrigin, read_creation_origin};
use wns_transfer::host::{TransferFactory, TransferSource};
use wns_transfer::import::{
...
pub struct Library<F: TransferFactory> {
    pub(crate) connection: Connection,
    _lock: File,
    pub root: PathBuf,
    pub(crate) namespace: String,
    /// The crate owns the library's behaviour but not the project type; the
    /// four methods that create or inspect one are written against the factory
    /// `webnovel-core` supplies. `webnovel_core::library::Library` is this type
    /// with the parameter fixed, so no caller names it.
...
impl<F: TransferFactory> Library<F> {
    ...
    pub fn open(root: impl AsRef<Path>) -> CoreResult<Self> {
        ...
        wns_storage::configure(&connection)?;
    ...
    pub fn save_endpoint_profiles(
        ...
        for draft in &drafts {
            ...
            let id = match &draft.id {
                ...
                None => format!(
                    "{}{}",
                    wns_providers::endpoints::ENDPOINT_PROVIDER_PREFIX,
                    Uuid::new_v4()
                ),
            };
    ...
    pub fn register(&mut self, project: &F::Session) -> CoreResult<()> {
        let metadata = project.metadata()?;
        let path = std::fs::canonicalize(project.project_path())?;
        self.register_metadata(&path, &metadata)
    }
    fn register_metadata(
        &mut self,
        path: &Path,
        metadata: &wns_storage::ProjectMetadata,
    ) -> CoreResult<()> {
    ...
    pub fn finish(&mut self, operation_id: &str, project: &F::Session) -> CoreResult<()> {
        let pending = self
            .operation(operation_id)?
            .ok_or_else(|| CoreError::new("InvalidRequest", "Unknown project operation."))?;
        if std::fs::canonicalize(&pending.final_path)? != project.project_path()
            || read_creation_origin(project.project_path())? != pending.origin
        {
            return Err(CoreError::new(
                "InvalidProject",
    ...
    pub fn create(&mut self, operation_id: &str, title: &str) -> CoreResult<F::Session> {
        let pending = self.begin(operation_id, "create", title, None)?;
        let project = F::create_staged(
    ...
    fn finish_import_receipt(
        ...
        if pending.kind != "import"
            ...
            || read_creation_origin(path)? != pending.origin
