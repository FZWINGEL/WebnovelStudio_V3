...
/// block un-movable: a method or helper that names the actor cannot travel to a
/// crate that does not own the actor. Four methods, measured from the bodies
/// rather than guessed.
pub trait SourcePinHost {
...
pub fn persistent_for_discussion(
    db: &Connection,
    access: &ProjectAccess,
    document_id: &str,
    include: bool,
