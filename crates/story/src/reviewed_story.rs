...
/// What this module needs from the actor, and nothing else.
///
/// Four methods — the same four `SourcePinHost` and `HistoryHost` declare, and
/// the same four the doc predicted once the helpers had travelled. The bodies
/// below reach for `check_access` six times, `db` four, `db_mut` twice (both
/// chained across a newline, the measurement trap this migration keeps hitting)
/// and `fence_uncertain` twice.
///
