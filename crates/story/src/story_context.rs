...
pub fn freeze_reviewed_continuation_at(
    tx: &Connection,
    request: &FreezeReviewedContinuation,
    payload_hash: &str,
) -> CoreResult<FrozenContext> {
...
pub fn freeze_discussion_story_at(
    tx: &Connection,
    request: &FreezeStory,
    payload_hash: &str,
    retry_guidance: Option<&[wns_context::guidance::FrozenGuidance]>,
) -> CoreResult<FrozenContext> {
...
fn read_current_navigation_view(
    ...
    let view = match crate::memory::validate_navigation_view_record(
        db,
        view_id,
...
fn validate_navigation_view_record(
    ...
    let stored = crate::memory::validate_navigation_view_record(
        db,
        &view.reference.view_id,
        Some(&frozen.snapshot.snapshot_id),
    )?;
