...
pub fn encode_import_operation(
...
pub fn decode_import_operation(
    fingerprint: &str,
    operation_id: &str,
    source_path: PathBuf,
    title: String,
...
pub fn stage_v2_import<F: TransferFactory>(
    request: &V2ImportRequest,
    ...
    if staging.exists() {
        return resume_or_refuse_staging::<F>(&validated, staging, destination, origin, request);
    }
    let parent = staging
        .parent()
    ...
    let mut project = F::open_staging(staging, &request.title)?;
    write_creation_origin(staging, origin)?;
    let result = populate_project::<F>(&mut project, &validated, request, origin)?;
    drop(project);
    let _ = F::create_staged(staging, destination, &request.title, origin)?;
    Ok(result)
}

pub fn read_import_result(
    project_path: &Path,
    operation_id: &str,
    expected_source_sha256: &str,
    expected_request_sha256: &str,
) -> CoreResult<V2ImportResult> {
    let origin = read_creation_origin(project_path)?;
...
pub fn recover_import_staging<F: TransferFactory>(
    staging: &Path,
    destination: &Path,
    title: &str,
    origin: &CreationOrigin,
...
fn populate_project<F: TransferFactory>(
    project: &mut F::Staging,
...
fn resume_or_refuse_staging<F: TransferFactory>(
    validated: &ValidatedImport,
    staging: &Path,
    destination: &Path,
    origin: &CreationOrigin,
