...
fn schema24_upgrade_keeps_exact_historical_codex_packet_and_backup() {
    ...
    assert_eq!(schema_version(&path.join("project.sqlite3")), 32);
...
fn schema14_reader_floor_upgrade_preserves_exact_reviews_and_working_snapshots() {
    ...
    assert!(
        backups[0]
            ...
            .unwrap()
            .to_string_lossy()
            .starts_with("schema14-before-schema32-")
    );
...
fn schema2_backup_recovers_forward_with_document_view_and_epoch() {
    ...
    assert_eq!(schema_version(&target.join("project.sqlite3")), 32);
    assert_eq!(
        recovered
            .context_source_epoch()
...
fn schema30_reader_floor_migrates_with_noop_step_and_retains_backup() {
    ...
    assert_eq!(schema_version(&database), 32);
    ...
    assert!(
        backups[0]
            .file_name()
            .to_string_lossy()
            .starts_with("schema30-before-schema32-")
    );
