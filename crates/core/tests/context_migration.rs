...
fn schema24_upgrade_keeps_exact_historical_codex_packet_and_backup() {
    ...
    crate::legacy_schema::remove_schema38_features(&database).unwrap();
    database
        .execute_batch(
    ...
    assert_eq!(schema_version(&path.join("project.sqlite3")), 38);
...
#[test]
fn schema35_to38_preserves_workshop_state_and_context_packet_bytes() {
    ...
    let packet = match project
        .prepare_context(PrepareContext {
            lookup: None,
            access: access.clone(),
            operation_id: "schema36-retained-packet".into(),
            snapshot_id,
            instruction: "Keep the relationship context grounded.".into(),
            mandatory_handles: Vec::new(),
            transient_mandatory_handles: None,
            safe_brief: None,
            scope: None,
        ...
        .expect("prepare retained packet")
    {
        PreparationResult::Prepared { packet, .. } => *packet,
    ...
    state.preferences.push(WorkshopPreference {
        ...
        family: "continuity".into(),
        meaning: "Keep relationship evidence explicit.".into(),
        examples: "Exact endpoint identity".into(),
        timing: "During exploration".into(),
        polarity: PreferencePolarity::Neutral,
    ...
    project
        ...
        .expect("save retained workshop state");
    let packet_id = packet.receipt.packet_id.clone();
    drop(project);

    let database_path = path.join("project.sqlite3");
    let database = Connection::open(&database_path).unwrap();
    let workshop_before: (i64, String, String) = database
        .query_row(
        ...
        .unwrap();
    let packet_before: (String, String, String) = database
        .query_row(
    ...
    assert_eq!(schema_version(&database_path), 38);
    ...
    let restored = reopened
        ...
        .expect("reopen retained packet");
    assert_eq!(restored.receipt.input_hash, packet_before.2);

    let database = Connection::open(&database_path).unwrap();
    let workshop_after: (i64, String, String) = database
        .query_row(
            "SELECT version,state_json,state_hash FROM workshop_state WHERE singleton=1",
            [],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
        ...
        .unwrap();
    let packet_after: (String, String, String) = database
        .query_row(
            "SELECT packet_json,packet_hash,input_hash FROM context_packets WHERE id=?",
            [&packet_id],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
        ...
        .unwrap();
    assert_eq!(workshop_after, workshop_before);
    assert_eq!(packet_after, packet_before);
    ...
    assert!(
        backups[0]
            ...
            .unwrap()
            .to_string_lossy()
            .starts_with("schema35-before-schema38-")
    );
    assert_eq!(schema_version(&backups[0]), 35);
}

#[test]
fn schema36_reader_floor_rejects_missing_typed_packet_carrier() {
    let temp = TempDir::new("schema36-reader-floor");
    let path = temp.child("project");
    let (project, _, _, _) = setup_project(&path);
    drop(project);
    ...
    database
        .execute_batch(
            "ALTER TABLE context_packets DROP COLUMN input_hash;
             PRAGMA user_version=36;",
        )
        .unwrap();
    drop(database);

    let error = match ProjectSession::open(&path) {
        Ok(_) => panic!("schema36 without the packet carrier must be rejected"),
        Err(error) => error,
    };
    assert_eq!(error.code, "UnsupportedSchema");
    assert!(error.detail.contains("schema 36"));
}

#[test]
fn schema27_upgrade_preserves_historical_dynamic_author_packet_without_projection() {
    use webnovel_core::context::packet::ProviderBinding;

    let temp = TempDir::new("dynamic-author-packet");
    let path = temp.child("project");
    let (project, access, _, saved) = setup_project(&path);
    ...
    let started = project
        .start_discussion(StartDiscussion {
            access: access.clone(),
            operation_id: "historical-dynamic-author".into(),
            expected: saved.head,
            instruction: "Find the saved passage detail.".into(),
        ...
        .expect("start dynamic author discussion");
    let packet = started.packet;
    assert_eq!(packet.options.provider_binding.as_ref(), Some(&binding));
    assert!(packet.receipt.lookup.is_some());
    assert!(
        packet
            .receipt
            .lookup
    ...
    assert_eq!(serde_json::to_vec(&restored).unwrap(), packet_bytes);
    assert_eq!(
        serialized_input(&restored.messages, &restored.options).unwrap(),
        input
    );

    let archive = temp.child("dynamic-author.wnsbackup");
    create_backup(&reopened, &archive).expect("backup historical dynamic author packet");
    let recovered = recover_backup(
        &archive,
        &temp.child("recovered"),
        "Recovered dynamic author",
    )
    .expect("recover historical dynamic author backup");
    let recovered_db = Connection::open(temp.child("recovered/project.sqlite3")).unwrap();
    let recovered_json: String = recovered_db
        .query_row(
            "SELECT packet_json FROM context_packets WHERE id=?",
            [&packet.receipt.packet_id],
            |row| row.get(0),
        )
        .expect("read recovered historical dynamic author packet");
    assert_eq!(recovered_json.as_bytes(), packet_bytes.as_slice());
    let recovered_packet: webnovel_core::context::packet::CompiledPacket =
        serde_json::from_str(&recovered_json).unwrap();
    assert_eq!(
        serialized_input(&recovered_packet.messages, &recovered_packet.options).unwrap(),
        input
    );
    let recovered_input_hash: String = recovered_db
        .query_row(
            "SELECT input_hash FROM context_packets WHERE id=?",
            [&packet.receipt.packet_id],
            |row| row.get(0),
        )
        .expect("read recovered packet input hash");
    assert_eq!(recovered_input_hash, packet.receipt.input_hash);
    drop(recovered_db);
    drop(recovered);
...
fn schema14_reader_floor_upgrade_preserves_exact_reviews_and_working_snapshots() {
    ...
    db.execute_batch("DROP TABLE snapshot_navigation_views; DROP TABLE memory_view_sources; DROP TABLE memory_views; DROP TABLE memory_results; DROP TABLE memory_jobs; ALTER TABLE snapshot_sources DROP COLUMN reader_position")
        .unwrap();
    crate::legacy_schema::remove_schema38_features(&db).unwrap();
    ...
    assert!(
        backups[0]
            ...
            .unwrap()
            .to_string_lossy()
            .starts_with("schema14-before-schema38-")
    );
...
fn schema2_backup_recovers_forward_with_document_view_and_epoch() {
    ...
    assert_eq!(schema_version(&target.join("project.sqlite3")), 38);
    assert_eq!(
        recovered
            .context_source_epoch()
...
#[test]
fn schema30_reader_floor_migrates_with_noop_step_and_retains_backup() {
    let temp = TempDir::new("schema30-reader-floor");
    let path = temp.child("source");
    let (project, _access, document, saved) = setup_project(&path);
    drop(project);
    ...
    connection
        ...
        .expect("mark synthetic schema30 database");
    drop(connection);

    let reopened = ProjectSession::open(&path).expect("schema30 database upgrades");
    assert_eq!(schema_version(&database), 38);
    ...
    let backups = fs::read_dir(path.join("migrations"))
        .expect("read migration backup directory")
        .filter_map(Result::ok)
        .collect::<Vec<_>>();
    assert_eq!(backups.len(), 1);
    assert!(
        backups[0]
            .file_name()
            .to_string_lossy()
            .starts_with("schema30-before-schema38-")
    );
