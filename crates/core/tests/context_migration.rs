...
fn schema2_upgrade_preserves_documents_view_state_epoch_and_durable_pre_upgrade_backup() {
    ...
    assert_eq!(schema_version(&path.join("project.sqlite3")), 8);
    assert_eq!(
        upgraded
...
#[test]
fn schema3_upgrade_to_schema8_preserves_frozen_snapshot_and_useful_backup() {
    ...
    assert_eq!(schema_version(&path.join("project.sqlite3")), 8);
    let connection =
        Connection::open(path.join("project.sqlite3")).expect("open migrated schema8 database");
    let discussion_tables: i64 = connection
        ...
        .expect("check schema8 discussion tables");
...
#[test]
fn schema2_backup_recovers_forward_to_schema8_with_document_view_and_epoch() {
    ...
    assert_eq!(schema_version(&target.join("project.sqlite3")), 8);
    assert_eq!(
        recovered
            .context_source_epoch()
