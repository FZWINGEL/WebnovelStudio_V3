...
impl OwnedProject {
    pub(super) fn handle_export(&mut self, command: ExportCommand) {
        macro_rules! respond {
        ...
        match command {
            ...
            ExportCommand::Install(access, preview, target, basename, reply) => {
                respond!(
                    reply,
                    self.export_prepared_draft(access, &preview, &target, &basename)
                );
            }
            ExportCommand::Read(access, id, reply) => {
                respond!(
                    ...
                    self.check_access(&access).and_then(|()| {
                        read_export_record(self.db()?, &access.operation_namespace, &id)
                    })
                );
            }
        }
    ...
    fn export_prepared_draft(
        ...
        self.check_access(&access)?;
        let existing = validate_export_preview(self.db()?, &self.info, &access, preview, basename)?;
        ...
        crate::transfer::install_export_file(&self.path, target, projected.text.as_bytes())?;
        ...
        match record_result {
            ...
            Err(error) => {
                ...
                if error.code == "UncertainOutcome" {
                    ...
                    self.fence_uncertain(&uncertain);
                }
                Err(CoreError::new(
...
pub(crate) fn validate_export_storage(connection: &Connection) -> CoreResult<()> {
...
pub(crate) fn validate_basename(value: &str) -> CoreResult<()> {
