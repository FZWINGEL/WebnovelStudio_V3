...
pub(super) fn hold_after_commit_before_ack(operation_id: &str) {
    ...
    ) && let Some(root) = std::env::var_os("WNS_UNIT_CRASH_ROOT")
    {
        ...
        marker
            .write_all(b"COMMIT returned successfully; no document acknowledgment has been sent")
            .unwrap();
...
fn crash_child() {
    ...
    if value.get("preparedId").is_some() {
        ...
        let mut request: context_packets::PrepareContext = serde_json::from_slice(&json).unwrap();
        request.access = access;
...
fn kill_after_commit_before_ack_recovers_once() {
    ...
    let project = ProjectSession::create(root.join("project"), "Crash recovery fixture").unwrap();
...
fn killed_context_prepare_after_commit_retries_the_same_packet_once() {
    ...
    std::fs::create_dir(&root).unwrap();
    let project = ProjectSession::create(root.join("project"), "Context crash fixture").unwrap();
    let access = project.documents().attach("parent".into()).unwrap();
    let document = project
        .documents()
        .create(CreateDocument {
...
fn killed_restore_after_commit_recovers_historical_result_once() {
    ...
    std::fs::create_dir(&root).unwrap();
    let project = ProjectSession::create(root.join("project"), "Restore crash fixture").unwrap();
    let access = project.documents().attach("parent".into()).unwrap();
