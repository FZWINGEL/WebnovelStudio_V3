...
use crate::projects::story_context::{FrozenContext, SearchMode};
...
pub(super) fn validate_allowance(allowance: &LookupAllowance) -> CoreResult<()> {
    allowance
        .validate()
        .map_err(|error| CoreError::new("InvalidLookupAllowance", &error.to_string()))
}

pub(super) fn parse_response(raw: &str) -> CoreResult<StoredResult> {
...
pub(super) fn claim(
    ...
    let packet = crate::projects::context_packets::validated_packet_record(tx, &current.packet_id)?;
...
pub(super) fn mark_chain_stopped(tx: &Connection, run_id: &str) -> CoreResult<()> {
    tx.execute(
...
pub(super) fn execute_read(
    ...
    match request {
        ...
        LookupRead::Read {
            handle, block_ids, ..
        } => match crate::projects::story_context::read_source(db, frozen, handle) {
            Ok(source) => {
...
fn execute_search(
    ...
    match crate::projects::story_context::search_frozen(db, frozen, query.trim(), mode, limit) {
...
pub(super) fn read_summary(db: &Connection, run_id: &str) -> CoreResult<Option<LookupRunSummary>> {
...
pub(crate) fn validate_storage(db: &Connection) -> CoreResult<()> {
    let mut statement = db.prepare(
    ...
    ) in rows
    {
        ...
        let (frozen, frozen_namespace) =
            super::story_context::validated_snapshot_record(db, &snapshot_id)?;
        ...
        let root_packet = crate::projects::context_packets::validated_packet_record(db, &root.3)?;
...
fn lookup_packet_request(
    db: &Connection,
    packet_id: &str,
) -> CoreResult<crate::projects::context_packets::PrepareContext> {
...
fn validate_reads(db: &Connection) -> CoreResult<()> {
    ...
    ) in rows
    {
        ...
        let child_packet =
            crate::projects::context_packets::validated_packet_record(db, &child_packet_id)?;
