...
impl OwnedProject {
    fn current_background_access(&self) -> CoreResult<ProjectAccess> {
        if self.needs_reopen {
            return Err(recovery_required());
        }
        self.access.clone().ok_or_else(recovery_required)
    }

    fn active_background_records(
        &self,
        access: &ProjectAccess,
    ) -> CoreResult<Vec<BackgroundWorkItem>> {
        let db = self.db().map_err(|_| recovery_required())?;
    ...
    fn validate_expected_background_work(
        &self,
        access: &ProjectAccess,
    ...
    pub(super) fn background_work(&self) -> CoreResult<BackgroundWork> {
        let access = self.current_background_access()?;
        Ok(BackgroundWork {
            items: self.active_background_records(&access)?,
            errors: Vec::new(),
        })
    }
    ...
    pub(super) fn stop_background_work(
        ...
        let access = self.current_background_access()?;
        self.validate_expected_background_work(&access, &expected)?;
    ...
    pub(super) fn interrupt_background_work(
        &mut self,
        expected: BackgroundWork,
    ) -> CoreResult<BackgroundWork> {
        let access = self.current_background_access()?;
